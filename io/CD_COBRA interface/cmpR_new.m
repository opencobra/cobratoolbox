...
%  listOfFound --- a list of reactions in the focusing model that are
%  present in the reference COBRA model.

%  listOfNotFound --- a list of reactions in the focusing model that are
%  NOT present in the reference COBRA model.

%  found_rxns_and_mets --- the matches of reactions in the parsed model in
%  the COBRA model and list of substrates and products for the reaction.

%  list_of_rxns_not_present_in_ReferenceModel --- a list of reactions in
%  the reference model that are NOT included in the focusing model.



%% Procedures in the running of the funcion:
% first, the function search the reactions COBRA model for the reactions
% in the CD model; Second, the function search the reactions CD model for the reactions
% in the COBRA model.
...
for r_p=1:length(rxnList_p(:,1)); % the number of the iteration is equal to the number of reactions in the parsed model.
    ...
    for numOfID=1:c_original;  % three different forms of ID, names;
        ...
        if ~isempty(find(~cellfun('isempty',strfind(rxnList_m(:,1),rxn{1})))) % rxnList_m a COBRA model;
            ...
            results.listOfFound(nF,1:c_original)=rxnList_p(r_p,1:c_original);  % rxnList_p parsed model; r_p is the reactin number in the matrix.
            ...
            if ind~=0
                ...
                c_index(a,1:length(ind))=ind;  % indx contions all the possible reaction IDs in the COBRA model for each reaction in the parsed model(rxnList_p(r_p))
                ...
                for rxnNUM=1:length(ind) % the reaction ID in the identified reaction list
                    ...
                    for rxnN=1:length(i) % list of number of reactions;
