...
[tmp,fieldList] = regexp(notesField,['<' tag '>.*?</' tag '>'], ...
    'tokens', 'match');
...
for i = 1:length(fieldList)
    ...
    if (regexp(fieldStr,'GENE_ASSOCIATION'))
        gprStr = regexprep(strrep(fieldStr,'GENE_ASSOCIATION:',''), ...
            '^(\s)+','');
        grRule = gprStr;
        [genes,rule] = parseBoolean(gprStr);
    elseif (regexp(fieldStr,'GENE ASSOCIATION'))
        gprStr = regexprep(strrep(fieldStr,'GENE ASSOCIATION:',''), ...
            '^(\s)+','');
        grRule = gprStr;
        [genes,rule] = parseBoolean(gprStr);
    ...
    elseif (regexp(fieldStr,'CHARGE'))
        charge = str2num(regexprep(strrep(fieldStr,'CHARGE:',''), ...
            '^(\s)+',''));
    elseif (regexp(fieldStr,'AUTHORS'))
        if isempty(citation)
            citation = strcat(regexprep(strrep(fieldStr,'AUTHORS:',''), ...
                '^(\s)+',''));
        else
            citation = strcat(citation, ';', ...
                regexprep(strrep(fieldStr,'AUTHORS:',''),'^(\s)+',''));
        end
