...
function [text,number,changedStrLines,col]=updateCD(text,ref,note)
...
% update the species name according to a reference list of the names.
col_name=2; % the column 2 contains the metabolite names.



pool={'<species (\w*)','<celldesigner:name>'};
...
for i=1:length(pool);
    ...
    for ln=1:length(text);
        ...
        line_species=regexpi(text(ln).str,str); % find the line that conatins the string ('str').
