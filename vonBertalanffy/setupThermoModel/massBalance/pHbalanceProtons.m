...
for n=1:nRxn
    ...
    if model.SIntRxnBool(n) && ~massImBalancedBool(n)
        ...
        if aveHbound*model.S(:,n)>(eps*1e4)
            ...
            warning('pHbalanceProtons:Failed',['Failure to proton balance. Reaction ' model.rxns{n} ', #' int2str(n)])
        end
    ...
    if aveHbound*model.S(:,n)>(eps*1e4)
            ...
            warning('pHbalanceProtons:UnbalancedRxn', ['Reaction ' model.rxns{n} ', #' int2str(n) ' not proton balanced. No thermodynamic data available to balance reaction.']) % Changed from error to warning - Hulda
    end
end
if any(any(isnan(model.S)))
