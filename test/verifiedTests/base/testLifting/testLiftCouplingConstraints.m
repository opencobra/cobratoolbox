...
%% gecko-style model - with pre-existing D and E fields
...
model.b = 0;
model.c = [0; 0; 0];
model.lb = [1; 0; 0]; % fluxes of reactions using enzymes in GECKO-style models is positive (no irreversible rxns)
...
model.dsense = repmat('L', numel(model.ctrs), 1);
...
model.evarub = 1e3*ones(numel(model.evars),1);
...
evarub_expt = [1e3*ones(3, 1); Inf*ones(4, 1)];
