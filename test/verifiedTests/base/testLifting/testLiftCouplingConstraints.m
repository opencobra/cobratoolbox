...
%% gecko-style model - with pre-existing D and E fields, including constraints with equalities and more than 2 variables and 1 coefficient to lift
...
% the toy model only already minimizes the enzyme pool.
% for such small model this objective alone give an unique flux and
% extra variables vectors. to guarantee that, we must add extra constraints
n = numel(model.rxns);
nOrigExtrVar = numel(model.evars); % # of enzyme usage variables in GECKO
nOrigVar = n + nOrigExtrVar;
% * fix the variable of enzyme pool - upool to it's minimum values
OnlyMinPoolModel = model;
...
model.evarlb(3) = sol0.full(nOrigVar);
model.evarub(3) = sol0.full(nOrigVar);
% * minimize flux through all variables (flux and extra variables, i.e.
% enzyme usage)
model.c = ones(numel(model.rxns), 1);
model.evarc = ones(numel(model.evars), 1);

% solve before lifting
LP0b = buildOptProblemFromModel(model, 'true', struct());
sol0b = solveCobraLP(LP0b, struct()); % considers additional constraints from C, D and E when doing FBA

% lift
BIG = 1e3;
printLevel = 0;
model_lifted = liftCouplingConstraints(OnlyMinPoolModel, BIG, printLevel, true); % true for handling equalities

% solve after lifting.
% as done before for the unlifted model:
% * fix only the variable of enzyme pool - upool to it's minimum values
% we have to erase the 
LP1 = buildOptProblemFromModel(model_lifted, 'true', struct());
sol1 = solveCobraLP(LP1);
model_lifted.evarlb(3) = sol1.full(nOrigVar);
model_lifted.evarub(3) = sol1.full(nOrigVar);
% * minimize flux through all variables (flux and extra variables, i.e.
% enzyme usage)
model_lifted.c = ones(numel(model_lifted.rxns), 1);
model_lifted.evarc(1:nOrigExtrVar) = ones(nOrigExtrVar, 1);
LP1b = buildOptProblemFromModel(model_lifted, 'true', struct());
sol1b = solveCobraLP(LP1b, struct()); % considers additional constraints from C, D and E when doing FBA
...
D_expt = [zeros(1, 10);
          ...
          [zeros(1, 4), -100, zeros(1, 5)];
           ...
           [zeros(1, 9), 1]];
E_expt = zeros(1, 10);
evarlb_expt = [model.evarlb; -Inf*ones(7, 1)];
evarub_expt = [model.evarub; Inf*ones(7, 1)];
evarc_expt = [model.evarc; zeros(7, 1)];
evars_expt = [model.evars; {'z_1'; 'LIFT1_v2'; 'LIFT2_v2'; 'LIFT1_v1'; 'LIFT2_v1'; 'LIFT1_v2'; 'LIFT2_v2'}];
...
% THIS NEXT ASSERT SOMETIMES WORKS AND SOMETIMES FAILS!:
% it could be that the original model even after adding constraints
% has more than one possible solution vector
% Better to prove this with a whole gecko model?
% test if flux vector is the same
% assert(all(abs(sol1b.full(1:nOrigVar) - sol0b.full(1:nOrigVar)) < tol));
...
%% split and lift constraints > 2 variables when D and E are missing
model = createToyModelWDE();
model = rmfield(model, {'E', 'D', 'evars', 'evarNames', 'evarc', 'evarlb', 'evarub'});
...
%% split and lift constraints having > 2 variables and > 1 coefficient to be lifted
clear model
model.S = [1 -1 -1];
model.rxns = {'v1'; 'v2'; 'v3'};
model.mets = {'met1'};
