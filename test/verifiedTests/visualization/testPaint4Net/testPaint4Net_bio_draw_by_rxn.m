...
% Bioinformatics toolbox Needed for bio_draw_by_rxn & bio_draw_by_met and 
...
for i=1:length(Involved_mets)
    assert(sum(ismember(Involved_mets{i},Involved_mets_rxn_bio{i}))==length(Involved_mets{i}), 'Reactions do not match expected...');
end
...
for i=1:length(Dead_ends)
    assert(sum(ismember(Dead_ends{i},Dead_ends_rxn_bio{i}))==length(Dead_ends{i}), 'Reactions do not match expected...');
end
