% The COBRAToolbox: testXomicsToModel_fastCore.m
%
% Purpose:
%     - Drive the XomicsToModel multi-omics context-specific model extraction
%       pipeline end-to-end with the fastCore tissue-specific solver, on the
...
%     - Covers XomicsToModel, preprocessingOmicsModel, fastcc and fastCore.
%
...
if strcmp(getCobraTestMode(), 'fast')
    error(CBT_MISSING_REQUIREMENTS_ERROR_ID, ...
        'testXomicsToModel_fastCore is full-mode-only (heavyweight ~10 min); skipped in fast mode.');
end

% XomicsToModel and the fastCore extraction need an LP and a MILP solver
...
fileDir = fileparts(which('testXomicsToModel_fastCore'));
...
% extraction parameters (from the tutorial), with the fastCore tissue-specific solver
...
% the expected scale (reference: 1375 mets x 2222 rxns, 1235 genes)
refNmets = 1375; refNrxns = 2222; refNgenes = 1235; sizeTol = 0.05;
