%% _E.coli_ Core Model for Beginners (PART 2)
...
% growth-rate "FBAsolution.f" is listed as 0.8739 ${\textrm{hr}}^{-1}$. “FBAsolution.x” 
...
% 
% The flux values found in the structure "FBAsolution.x"  can be printed out 
% using the "printFluxVector(model,fluxData,nonZeroFlag,excFlag)" where the second 
% argument is a vector of the flux values, the nonZeroFlag only prints nonzero 
% rows (Default = false), and excFlag only prints exchange reaction fluxes (Default 
% = false). Examples of printing non-zero fluxes and exchange reaction only fluxes 
% are shown below. _[Timing: Seconds]_
...
% 
% 
% *Figure 7.* Screenshot of the network map of the _E.coli_ core model with 
% EX_glc(e) $\ge$ -10 $\textrm{mmol}\cdot {\textrm{gDW}}^{-1} \cdot {\textrm{hr}}^{-1}$ 
% and EX_o2(e) $\ge$ -30 $\textrm{mmol}\cdot {\textrm{gDW}}^{-1} \cdot {\textrm{hr}}^{-1}$.
% 
% As a cautionary note, the default condition for the _E.coli_ core model sets 
% the carbon source as glucose with an uptake rate of -10 $\textrm{mmol}\cdot 
% {\textrm{gDW}}^{-1} \cdot {\textrm{hr}}^{-1}$, the oxygen uptake is -1000 $\textrm{mmol}\cdot 
% {\textrm{gDW}}^{-1} \cdot {\textrm{hr}}^{-1}$ which implies an aerobic environment 
...
%% 4. The Subsystems of the _E.coli_ Core Model
...
% model. This subsystem is located in the upper right corner of the _E.coli_ core 
% map as shown below in Figure 8.
% 
% 
% 
% *Figure 8.* The location of the energy management subsystem and it's reactions 
...
% pathway with both phosphoglycerate kinase (PGK), and pyruvate kinase (PYK), 
% and in the tricarboxylic acid cycle with succinyl-CoA synthetase (SUCOAS). Through 
% these substrate level phosphorylation enzymes each molecule of glucose can potentially 
% add four molecules to the total cellular flux of atp[c]. 
% 
% The second mechanism for energy generation is oxidative phosphorylation through 
% the electron transport chain, which under aerobic conditions, produces the bulk 
% of the cell's atp[c]. In the simple core model, the electron transport chain 
% is used to transport protons (h[c]) from the cytoplasm across the cytoplasmic 
...
% 
% 
% *Figure 9.* Oxidative Phosphorylation and Transfer of Reducing Equivalents 
% [3].
% 
...
% power necessary to trigger the electron transport chain. The _E. coli_ core 
% model combines the electron transport chain into two reactions. In the first 
% of these two reactions, NADH16 catalyzes the oxidation of nadh[c] to form NAD+ 
% (nad[c]) while extracting four protons (h[c]) from the cytoplasm. It then transports 
% three protons to the extracellular space while combining the fourth proton with 
% a proton and two electrons from NADH to transform ubiquinone-8) (q8[c]) to its 
% reduced form ubiquinol-8 (q8h2[c]). Both q8[c] and q8h2[c] are oil soluble coenzymes 
% that can diffuse freely within the lipid environment of the cytoplasmic membrane 
% allows q8h2[c] to eventually transfer its two electrons and two protons to cytochrome 
% oxidase (CYTBD). The two protons (h[e]) are then transferred into the extracellular 
% space where they add to the proton-motive force. The two electrons from q8h2[c] 
% are then combined with two cytoplasmic protons and an oxygen atom, the terminal 
% electron acceptor, to form water. In this model, oxygen (o2[c]) spontaneously 
% diffuses from the environment into the cell through the spontaneous 02t reaction. 
% 
% With a proton-motive force now created by the pumping of protons from the 
% cytoplasm to the extracellular space, the reaction ATPS4r can synthesize atp[c] 
...
% 
% Finally, the ATP maintenance function (ATPM), which is set at 8.39 $\textrm{mmol}\cdot 
% {\textrm{gDW}}^{-1} \cdot {\textrm{hr}}^{-1}$ accounts for the energy (in form 
% of atp[c]) necessary to replicate a cell, including for macromolecular synthesis 
% (e.g., proteins, DNA, and RNA). Thus, for growth to occur in the _E.coli_ model, 
% the flux rate through ATPM must be greater than 8.39 $\textrm{mmol}\cdot {\textrm{gDW}}^{-1} 
% \cdot {\textrm{hr}}^{-1}$. If the model detects that ATPM has not reached its 
...
% 
% Now lets explore the flux through these reactions in aerobic conditons with 
% the glucose uptake set at -10 $\textrm{mmol}\cdot {\textrm{gDW}}^{-1} \cdot 
% {\textrm{hr}}^{-1}$ and the oxygen uptake at -30 $\textrm{mmol}\cdot {\textrm{gDW}}^{-1} 
% \cdot {\textrm{hr}}^{-1}$. _[Timing: Seconds]_
...
%% 
% Below in Figure 10 is screenshot showing these fluxes flowing through the 
% oxidative phosphorylation section of the core map (upper right corner).  In 
% this figure we can see the electrons from nadh[c] entering the electron transport 
...
% 
% 
% *Figure 10:* Close-up of the oxidative phosphorylation section of the _E.coli_ 
% core map in aerobic conditions with glucose as the sole carbon source (see Figure 
% 7).
%% 
% _*ATP Production*_
% 
...
% The atp[c] used by ATPM must be greater than or equal to 8.39 $\textrm{mmol}\cdot 
% {\textrm{gDW}}^{-1} \cdot {\textrm{hr}}^{-1}$ to allow the cell to grow. Finally 
% the biomass function shows that 52.27 $\textrm{mmol}\cdot {\textrm{gDW}}^{-1} 
% \cdot {\textrm{hr}}^{-1}$(0.873922 x 59.81) is used for the cell's biosynthesis 
% needs. 
% 
...
total_adp_flux = sum(vP)
%% 
% These results show that the amount of atp[c] flux in the cell equals the amount 
% of adp[c] flux. Thus, the adp[c]/atp[c] flux ratio is 1. This is also true for 
% nadp[c]/nadph[c] and the nad[c]/nadh[c] flux ratios. 
% 
% Another way to explore the ATPS4r's ability to produce atp[c] is through the 
% use of robustness analysis [12]. Assuming that the objective function is the 
% biomass function (growth-rate), then the following simulation illustrates that 
% the maximum atp[c] flux that can be supported by ATPS4r under aerobic conditions 
...
%% 
% This graph shows the entire capability of ATPS4r when the carbon source glucose 
% has a maximum uptake rate greater than or equal to  -10 $\textrm{mmol}\cdot 
% {\textrm{gDW}}^{-1} \cdot {\textrm{hr}}^{-1}$. If we start at the left of this 
...
% a maximum of 0.8738 ${\textrm{hr}}^{-1}$ when the ATPS4r flux level reaches 
% 45.54 $\textrm{mmol}\cdot {\textrm{gDW}}^{-1} \cdot {\textrm{hr}}^{-1}$. After 
% the maximum growth-rate has been achieved the cell then needs to find ways to 
% recycle the extra ATP. This can be seen below by fixing the flux through ATPS4r 
% to a value greater than 45.54 $\textrm{mmol}\cdot {\textrm{gDW}}^{-1} \cdot 
% {\textrm{hr}}^{-1}$. _[Timing: Seconds]_
...
drawFlux(map, model, FBAsolution.x, options); % Draw the flux values on the map "target.svg"
%% 
% If we compare these results with the previous fluxes calculated for the optimized 
% cell performance under aerobic conditions with a similar glucose carbon source 
% uptake, we can see the differences in atp[c] flux distribution.To start with 
% it can be seen that the flux through ATPM increases (13.74 > 8.39). Notice that 
% ADK1 has been activated to recycle atp[c] to adp[c]. Since the growth-rate decreases, 
% we would also expect the flux used by the biomass function to decrease along 
% with other parts of the cell by selecting alternate pathways to help absorb 
% the extra atp[c]. This is illustrated in the core metabolic map shown below.
% 
% 
% 
% *Figure 11.* A screenshot of the core map with ATPS4r fixed at 60$\textrm{mmol}\cdot 
% {\textrm{gDW}}^{-1} \cdot {\textrm{hr}}^{-1}$. 
%% 
% _*NADH Production*_
% 
...
%% 
% Note that in this case, the only consumer of nadh[c] is NAD16 which is the 
% beginning of the electron transport chain. The producing reactions, as we will 
% discuss later, are primarily located in the glycolysis and TCA pathways. Note 
% that for this core model, the biomass function is also listed as a producer. 
...
% (3.547) to yielding a total nadh[c] biomass flux of 3.0998 $\textrm{mmol}\cdot 
% {\textrm{gDW}}^{-1} \cdot {\textrm{hr}}^{-1}$. This can also be calculated using 
...
%% 
% Due to the simplicity of the _E.coli_ core model, most of the nadph[c] is 
% consumed by the biomass function (0.873922 x 13.0279 = 11.385) to support the 
% cell's biosynthesis needs. The other consumer is the nitrogen metabolism (GLUDy). 
% On the other hand, nadph[c] is produced by reactions in the oxidative phosphorylation 
% pathways, pentose phosphate pathway, and the TCA cycle. It is worth pointing 
% out that in the larger models, that incorporate most of the cells biosynthesis 
% pathways, the number of reactions consuming nadph[c] could be very large. _[Timing: 
% Seconds]_
%% 
% _*Anaerobic Respiration*_
% 
% Now let's turn our attention to anaerobic cell operation. During aerobic respiration, 
% oxygen is the terminal electron acceptor for the electron transport chain, which 
% yields the bulk of atp[c] required for biosynthesis. Anaerobic respiration refers 
% to respiration without molecular oxygen. For anaerobic respiration, _E. coli_ 
% only generates atp[c] by substrate level phosphorylation. Glycolysis results 
% in the net production of two atp[c] per glucose by substrate level phosphorylation, 
...
% 
% 
% *Figure 12.* Network map of the _E.coli_ core model with glucose as the carbon 
% source (EX_glc(e) $\ge$ -10 $\textrm{mmol}\cdot {\textrm{gDW}}^{-1} \cdot {\textrm{hr}}^{-1}$) 
% in an anaerobic environment (EX_o2(e) $\ge$ 0 $\textrm{mmol}\cdot {\textrm{gDW}}^{-1} 
% \cdot {\textrm{hr}}^{-1}$).
% 
...
%% 
% Now let's look at the fomulas for these reactions to understand what is happening 
% in this condition. _[Timing: Seconds]_
...
%% 
% So one question that could be asked is this anaerobic environment is, where 
% is the nadh[c] produced and where is it consumed. Using "surfNet" we can find 
% out. _[Timing: Seconds]_
...
% 
% Now let's explore the production of atp[c] in an anaerobic environment. _[Timing: 
% Seconds]_

surfNet(model, 'atp[c]',0,FBAsolution.x,1,1)
%% 
% As can be seen above, the production of atp[c] is exclusively through substrate 
% phosphorylation (ACKr, PGK, PYK).
% 
% Finally, the nadph[c] producers and consumers are shown below. _[Timing: Seconds]_

surfNet(model, 'nadph[c]',0,FBAsolution.x,1,1)
%% 
% Note that the primary producer of nadph[c] in this anaerobic environment is 
% THD2, which converts the surplus nadh[c] to nadph[c].
...
% Glycolysis is the metabolic pathway in the _E.coli_ core model that converts 
...
% of the glycolysis pathway on the _E.coli_ core map is highlighted in the Figure 
% 13.
% 
% 
% 
% *Figure 13.* The location of the glycolysis pathway subsystem reactions are 
% highlighted in blue on the _E.coli_ core map [3].
% 
% A table showing the reactions associated with the glycolysis pathway can be 
% extracted from the core model as follows: _[Timing: Seconds]_
...
% 
% In addition to providing some atp[c] through substrate phosphorylation (PGK 
% and PYK), the glycolysis pathway also proves a major source of nadh[c] (GAPD) 
% that is used to power the electron transport chain. It also supplies several 
% key precursors needed for the biosynthesis pathways. These precursors include: 
% D-Glucose 6-phosphate (g6p[c]) a precursor for sugar nucleotides, D-Fructose 
...
% 
% Visualizing the flux though the glycolysis pathways can be seen by using the 
% draw package available with COBRA Toolbox. This is illustrated in the Matlab 
...
% carbon source (EX_fru(e) $\ge$ -10 $\textrm{mmol}\cdot {\textrm{gDW}}^{-1} \cdot 
% {\textrm{hr}}^{-1}$) in an anaerobic environment (EX_o2(e) $\ge$ 0 $\textrm{mmol}\cdot 
% {\textrm{gDW}}^{-1} \cdot {\textrm{hr}}^{-1}$).
% 
...
% 
% Using the COBRA Toolbox, it is possible to create a table of reactions and 
% their flux values for both glycolysis supported carbon sources, glucose and 
% fructose. This is illustrated below. _[Timing: Seconds]_
...
% from the carbon source at the top left of the metabolic maps down the glycolysis 
% pathway to form pyruvate in the lower right. In aerobic conditions, part of 
% the flux is diverted to the G6PDH2r entrance to the pentose phosphate pathways. 
% For the anaerobic case, the flux is only diverted to the lower half of the pentose 
% phosphate pathway (TKT2) to produce the pentose phosphate pathway precursors. 
% Also note that the flux through GAPD has almost doubled since the number of 
% g3p[c] metabolites leaving the FBA and TPI reaction are double the number of 
% fdp[c] metabolites entering FBA. This is possible since the output of FBA provides 
% both a molecule of g3p[c] and a molecule of dhap[c]. The dhap[c] is rapidly 
% converted to g3p[c] thus creating the effect of doubling the g3p[c] entering 
% GAPD. A more detailed understanding of the fluxes through glycolysis using the 
% COBRA toolbox is left as an exploration opportunity for the reader.