%% _E.coli_ Core Model for Beginners (PART 1)
...
% # The subsystems of the _E.coli_ core model including sections on A) energy 
...
% 
% Through the use of genome-scale metabolic network reconstructions, Flux Balance 
% Analysis (FBA) [8] can be used to calculate the flow of metabolites through 
% a metabolic network. This capability makes it possible to predict the growth-rate 
% of an organism and/or the rate of production of a given metabolite. It is important 
% that it is understood that FBA has limitations! It does not use kinetic parameters, 
% thus it cannot predict metabolite concentrations. It is also only capable of 
% determining fluxes at steady state. Finally, traditional FBA does not account 
% for regulatory effects such as the activation of enzymes by protein kinases 
% or regulation of gene expression. Therefore, it's predictions may not always 
% be accurate.
% 
...
% to as the _E.coli_ core model, is a great model to explore the analysis and 
% exploration tools available through the COBRA toolbox which is the purpose of 
% this tutorial.  The metabolic map of the _E.coli_ core model is shown below 
...
% 
% A metabolic reconstruction consists of a collection of genes, reactions which 
% represent proteins (enzymes), metabolites, a stochiometric matrix that defines 
% the relationship between the reactions and metabolites, reaction constraints, 
...
% the simplified  _E.coli_ core model there are only two compartments represented; 
...
% 
% The COBRA models also include Boolean rules for each reaction describing the 
% gene-reaction relationship. For example, ‘gene1 and gene2’ indicate that the 
% two gene products are part of an enzyme whereas ‘gene1 or gene2’ indicate that 
% the two gene products are isozymes that catalyze the same reaction.The gene-protein-reaction 
% associations (GPRA) for a few reactions are shown in Figure 2. Each GPRA is 
% composed of a gene locus, a translated peptide (mRNA), and functional proteins 
% that work together to make a single reaction (enzyme). At the top of each GPRA 
% is a portion of the genomic context that is highlighted. Genes in this figure 
% are designated by their locus name and represented by light blue boxes. The 
% translated peptides are represented by the purple boxes, the functional proteins 
% are represented by red ovals, while the reactions are labeled dark blue boxes. 
% As can be seen in the figure, isozymes include two different proteins that are 
% connected to the same reaction. For the case of proteins with multiple peptide 
% subunits, the peptides are connected with an '&' sign above the protein. For 
% complexes of many functional proteins, the proteins are also connected with 
...
% 
% 
% *Figure 2.* Examples of the gene-protein-reaction associations from the _E. 
% coli_ core model [3].
%% 
% Now let's start to use the COBRA toolbox to begin exploring the _E.coli_ core 
% model. The first thing that needs to be done is to load the model into the Matlab 
% work environment. This can be achieved by loading the Matlab version of the 
% model (.mat) into Matlab. This model is available in the downloaded COBRA toolbox 
% software. _[Timing: Seconds]_
...
%% 
% After you load the _E.coli_ core model into the Matlab, you should be able 
% to look at the MATLAB workplace and see that the model is loaded as shown in 
% Figure 3.
% 
% 
% 
% *Figure 3.* Matlab workspace after the _E.coli_ core model has been loaded 
% into Matlab.
%% 
% Perhaps the easiest way to access all the information in the COBRA model is 
% to print out a spreadsheet that contains all the information stored in the model. 
% This can be accomplished using the "writeCbModel" function. _[Timing: Seconds]_
...
% 
% 
% *Figure 4.* Screenshot of "core_model.xls" showing a portion of the spreadsheet 
% with both reactions and metabolites of the model.
%% 
...
% 
% 
% *Figure 5.* The screenshot of a "drawflux" produced _E.coli_ core model map.
% 
% This SVG map can be read with most browsers and is very easy to use to search 
% and explore the _E.coli_ network.
%% 
% *2.A. Genes* 
% 
% Now let's begin by exploring some of the Matlab code and COBRA toolbox functions 
% that can be used to extract information about the genes from the model. Below 
% are a collection of COBRA Toolbox functions that retrieve key information from 
% the genes in the model. The genes are only represented by their gene locus number 
% (e.g. b2097). The gene name can be achieved with a quick search of the EcoCyc 
% website (https://www.ecocyc.org/). To start with, the genes included in the 
% model and their geneIDs are stored in the "model.genes" structure. The "findGeneIDs"  
% COBRA Toolbox function can be used to pull the geneID from the model structure.  
% The first 10 genes in the model, and their geneIDs, can be printed out as follows. 
% _[Timing: Seconds]_
...
%% 
% For the case of finding a single geneID from the model, the gene locus number 
% needs to be included in single quotes, such as 'b_number '. _[Timing: Seconds]_

findGeneIDs(model, 'b0116')
%% 
% Now, to find the reactions that are associated with a given gene, you can 
% use the "findRxnsFromGenes" function as shown below. _[Timing: Seconds]_
...
% *2.B. Reactions*
% 
% Reactions and their rxnIDs are stored in the "model.rxns" structure with the 
% reaction names being stored in "model.rxnNames." The COBRA Toolbox function 
...
%% 
% To find the formula of the reaction, use the "printRxnFormula" function. _[Timing: 
% Seconds]_

printRxnFormula(model,'ENO');
%% 
% To find the genes that are associated with a given reaction, you can use the 
% "findGenesRxns" function. _[Timing: Seconds]_

[geneList]=findGenesFromRxns(model,'ENO');
geneList{1:1}
%% 
% Finally, there are times when it is ncessary to find all the "reactant" metabolites 
% that feed a reaction as well as all the "product" metabolites that are produced 
% by the reaction. This can be achieved using the "surfNet" function as shown 
% below (there is a COBRA tutorial on this by Siu Hung Joshua Chan called "Browse 
% Networks in the Matlab Command Window Using surfNet"). This functions output 
% includes a listing of the reactants and products based on the reaction formulas. 
% It should be pointed out that in situations where a reaction becomes reversible, 
% a metabolite that is a reactant could become a product and a metabolite that 
% is a product could become a reactant. _[Timing: Seconds]_
...
% *2.C. Metabolites*
% 
% The metabolites included in the model and their metabolite IDs (metIDs) are 
% stored in the "model.mets" structure with the metabolite names being stored 
...
% 
% In constraint-based simulations, the system constraints are implemented in 
% two ways in COBRA models: 1) as reaction formulas that balance reaction input 
...
% $\textrm{mmol}\cdot {\textrm{gDW}}^{-1} \cdot {\textrm{hr}}^{-1}$with an upper 
% bound of +1000 $\textrm{mmol}\cdot {\textrm{gDW}}^{-1} \cdot {\textrm{hr}}^{-1}$. 
% On the other hand, irreversible reactions, except ATPM, are set with a lower 
% bound of 0 and an upper bound of +1000 $\textrm{mmol}\cdot {\textrm{gDW}}^{-1} 
% \cdot {\textrm{hr}}^{-1}$. The ATP maintenance reaction (ATPM) is set with a 
% lower bound of 0 and an upper bound of +8.39 $\textrm{mmol}\cdot {\textrm{gDW}}^{-1} 
% \cdot {\textrm{hr}}^{-1}$(this will be discussed later). All the exchange reactions, 
% except EX_glc(e), are set to allow secretion but not uptake, thus a lower bound 
% of 0 and an upper bound of +1000 $\textrm{mmol}\cdot {\textrm{gDW}}^{-1} \cdot 
% {\textrm{hr}}^{-1}$. To avoid confusion, it should be understood that all exchange 
% reactions, which are reactions that interface between the extracellular and 
% cytoplasmic space, assume that secretion is positive while uptake is labeled 
% negative.  Finally, the glucose exchange reaction, EX_glc(e), is set with a 
% lower bound of -10 $\textrm{mmol}\cdot {\textrm{gDW}}^{-1} \cdot {\textrm{hr}}^{-1}$ 
% and an upper bound of +1000 $\textrm{mmol}\cdot {\textrm{gDW}}^{-1} \cdot {\textrm{hr}}^{-1}$.
% 
...
% is automatically set to -10 $\textrm{mmol}\cdot {\textrm{gDW}}^{-1} \cdot {\textrm{hr}}^{-1}$. 
% 
% The results of the upper or lower bounds for a particular reaction can be 
% found by using the COBRA model structure, "model.lb" for the lower bound and 
% "model.ub" for the upper bound. _[Timing: Seconds]_
...
% parameter is the desired flux rate in $\textrm{mmol}\cdot {\textrm{gDW}}^{-1} 
% \cdot {\textrm{hr}}^{-1}$, and the fourth parameter can be 'u' - upper limit, 
% 'l' - lower limit, or 'b' - both (Default = 'b'). _[Timing: Seconds]_

model = changeRxnBounds(model,'EX_glc(e)',-5,'l');
printConstraints(model,-100, +100); % Showing the result of the change
%% 
% You can now see that the lower bound for glucose has been changed to -5 $\textrm{mmol}\cdot 
...
% that simulate biomass production. The precursors for the _E.coli_ core model 
...
%% 
% The objective function for the _E.coli_ core model is automatically set to 
% be the biomass reaction. Setting the biomass function to be the objective function 
% can also be done using the model = changeObjective(model,'reaction name'') as 
% shown below. _[Timing: Seconds]_
