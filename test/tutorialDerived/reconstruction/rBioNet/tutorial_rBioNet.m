...
% Luxembourg.*
% 
% rBioNet is a reconstruction tool that lets you assemble reconstruction in 
% a user friendly environment. In this tutorial you shall learn how you can use 
% this tool to either start a new reconstruction or load in an existing one, followed 
% by, its analysis. The tool consists of 3 main parts, i.e., metabolite creator, 
% reaction creator and reconstruction creator. The metabolite creator is used 
% to add in metabolites and its associated information, i.e., its elemental formula, 
% charge, identifiers (for e.g., KEGG ID, PubChem ID etc.) and other associated 
% attributes. Alternatively, a text file containing all the necessary information 
% in the same order as in the metabolite database can be loaded directly. The 
% reaction creator is used to formulate reactions and as stated before a text 
...
% 
% Click on the _Reaction View Table_ and then S_how All_, which shall show all 
...
% 
% 
% To visualize the 'Reconstruction Creator' window go to File and Open the model 
% creator. 
% 
...
% creator.
% 
% On the reconstruction creator, click on S_tatistics -> Reconstruction analyzer._ 
% A window called the 'Reconstruction Analyzer_'_ appears and is used to visualize 
...
% Editor.
% 
% _Go to file -> add text file -> with metabolite ->_ select the file tutorial_ureacycle_mets.txt 
...
% 
% Then, click New Reaction and enter the reaction information including: the 
% reaction abbreviation (ARGN), description (arginase), direction (Irreversible), 
% and confidence score (4). Click on More Properties to add additional information 
% (Notes, References, EC Number, KeggID).
% 
% 
% 
% Next go the "Metabolite View Table" and select a metabolite belonging to the 
% reaction. 
% 
...
% next section.
%% 
% Now, the tool checks for elemental and charge balancing, and provides a warning 
...
% Editor.
% 
% _Go to file -> add text file -> with reactions ->_ select the file _tutorial_ureacycle_rxns.txt_ 
...
% -> select the file _tutorial_ureacycle_genes.txt_ (provided in the tutorial 
% folder) 
...
% 
% 
% *Add an AND/OR rule to a group of genes:* Click 'Group AND' for groups selected 
...
%% 
% Once the GPR has been assigned and other edits done (for e.g., addition of 
% subsystem, further notes and references), click the Add Reaction button in the 
% Reconstruction Creator. 
%% 
...
% 
% Once you have finished with all the reactions, go to File -> Save -> As Reconstruction 
% Model. 
% 
% If you made errors while adding metabolites or reactions manually, you can 
% load the reaction or metabolite database .mat files directly into the MATLAB 
