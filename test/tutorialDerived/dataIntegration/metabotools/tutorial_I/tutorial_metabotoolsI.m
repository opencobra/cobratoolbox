...
% cell lines, CCRF-CEM and Molt- 4 cells. They will be generated by integrating 
...
%% 
% Apply the medium constraints previously defined using _setMediumConstraints._ 
% Note that this function also require the definition of the cell concentration 
...
%% 
% Define metabolites with missing experimental points but for which uptake and 
% secretion profiles need to be computed
...
%% 
% Use the function _defineUptakeSecretionProfiles_ to calculate the uptake and 
% secretion rate over the time of the culture for both condition (e.g. CCRF-CEM 
% and Molt- 4 cells)
...
% Use _calculateQuantitativeDiffs_ to calculate the sets of exchange reactions 
% with higher uptake and secretion in condition 1 than in condition 2.
% 
% Also adapt the condition uptake and secretion for the second condition. this 
% is sometimes necessary to allow the model to achieve a feasible flux.
...
%% 
% The models can also be compared by performing a sampling analysis using _performSampling_
...
%% 
% Use the function _summarizeSamplingResults_ to return the median of the flux 
% values from the two sampled models. The analysis can be limited to a specific 
