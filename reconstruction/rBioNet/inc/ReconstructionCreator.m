...
% Last Modified by GUIDE v2.5 02-Nov-2012 17:16:02
...
handles.server = [handles.server_light handles.server_text];% rBioNet class set server mode
...
del_rxn = data(unique(sel(:,1)),2);
for i = 1:length(del_rxn)
    handles.newrxns(strcmp(del_rxn{i},handles.newrxns(:,2)),:) = '';
end
...
model = fixExchangeRxns(data2model(handles.newrxns,handles.description));
if isempty(model)
    return;
end
...
% --------------------------------------------------------------------
function file_server_connect_Callback(hObject, eventdata, handles)
% hObject    handle to file_server_connect (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global rbionetGlobal;
...
function file_server_disconnect_Callback(hObject, eventdata, handles)
% hObject    handle to file_server_disconnect (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global rbionetGlobal;
...
% --------------------------------------------------------------------
function file_save_to_server_Callback(hObject, eventdata, handles)
% hObject    handle to file_save_to_server (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

data = handles.newrxns;
if isempty(data)
    msgbox('There are no reactions in this model to save.','No Reactions','error');
    return
end

global rbionetGlobal;
if ~strcmp(questdlg('Are you sure you want to save this model to server?',...
        'Save reconstruction','Yes','No','Yes'),'Yes')
    return;
end

info = rCreatorServerSave;
if isempty(info) 
    return;
end
if ~rbionetGlobal.saveReconInfo(info)
    return;
end



h = waitbar(0,'Saving reactions');

for i = 1:size(data,1)
     waitbar(i/size(data,1));
    if ~rbionetGlobal.reconSaveRxn(data{i,2},data{i,7},data{i,8},data{i,9},data{i,6},data{i,10},data{i,11},data{i,12})
        disp(['Reaction not saved: ' data{i,2}]);
    end
end
 close(h);
%nu er model klárt þá þarf að save-a reactionin.
