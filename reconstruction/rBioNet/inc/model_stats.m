...
if isempty(deadends)
    ...
    %detectDeadEnds returns the same metabolite twice.
    %That happens when we send a model through that has 
    %duplicate metabolites in the model structure (a corrupt/bad model).
    if size(deadends,1) ~= size(unique(deadends),1)
        
        msgbox(['The following metabolite(s) is/are possibly appearing more than once in this model. ';
            duplicate_values(deadends);
            ' ';
            'Possible solution:';'Close Reconstruction Analyzer '...
            'Save model once more (in the Reconstruction Creator) and open Reconstruction Analyzer again. '...
            'If this message dous not show again you are okey. '
            ],'Possible duplicate metabolites','warn');
    end
    deadends = unique(deadends);
