...
- **FR-007**: System MUST define the narrowest reproducibility check or test
  that proves the feature behavior.
- **FR-008**: System MUST state measurable performance or numerical-integrity
  ...
  diagnostics, or generated output volume.
...
- **SC-005**: [Project-specific verification, e.g., "The specified MATLAB
  reproducibility command completes with the expected solver status and
  residual labels"]
- **SC-006**: [Project-specific usability/performance criterion, e.g.,
  "Diagnostic output remains bounded to the documented summary table unless
  debug-level output is requested"]
