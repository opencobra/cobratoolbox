...
***********************************************************************************************************
*       The objective of this code (MustUU.gms) is used to identify the pair of fluxes, say j1 and j2, such
*       that one of the two increases in value for overproduction. The wild-type flux ranges are stored as parametric
...
$if not set gdxin $set gdxin MtoGUU
$GDXIN %myroot%%gdxin%
...
outer_obj..                                             z =e= sum(j$(can(j) and not constraint_flux(j) ), (w1(j)+w2(j) ) - (basemax(j)*y1(j)+basemax(j)*y2(j) ) );
...
primal_dual..                                           sum(j$(can(j) and not constraint_flux(j) ), (w1(j)+w2(j) ) ) =e= sum(j$ (constraint_flux(j)), b(j)*mu(j)) + sum(j, deltam(j)*LB(j)-deltap(j)*UB(j) );
...
dual_obj..                                              zdual =e= sum(j$ (constraint_flux(j)), b(j)*mu(j)) + sum(j$ (can(j) ), deltam(j)*LB(j)-deltap(j)*UB(j) );
