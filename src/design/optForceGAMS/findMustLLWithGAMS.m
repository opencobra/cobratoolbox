function [mustLL, pos_mustLL, mustLL_linear, pos_mustLL_linear] = findMustLLWithGAMS(model, minFluxesW, maxFluxesW, constrOpt, excludedRxns, mustSetFirstOrder, solverName, runID, outputFolder, outputFileName, printExcel, printText, printReport, keepInputs, keepGamsOutputs, verbose)
...
% findMustLL.gms is vendored under external/ (relocated out of src/); resolve by absolute path.
pathGamsFunction = [fileparts(which('initCobraToolbox')) filesep 'external' filesep 'design' filesep 'optForceGAMS' filesep gamsMustLLFunction];
