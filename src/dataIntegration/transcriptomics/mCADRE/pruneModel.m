function [tissueModel, cRes] = pruneModel(model, rankNonCore, coreRxn, zeroExpRxns, precursorMets, eta, tol, method)
...
%May 2017.
    ...
    while numel(rankNonCore) && count < cutoff % for testing
        ...
        if rStatus
            ...
            inactive_G = checkModelConsistency(tissueModel, method, r, tol);
            
            inactiveCore = intersect(inactive_G, coreRxn);
            inactiveNonCore = setdiff(inactive_G, inactiveCore);

            % Remove reactions with zero expression (previously penalized
            % in
