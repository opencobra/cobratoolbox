function [dATM, metAtomMappedBool, rxnAtomMappedBool, M2Ai, Ti2R] = buildAtomTransitionMultigraph_old(model, rxnfileDir, options)
...
for i=1:nAtransInstances
    ...
    if ~strcmp(ATN.atoms{ah(i)}(end),ATN.atoms{at(i)}(end))
        disp(ATN.atrans{i,1})
        error('Substrate and product element in atom transition do not match')
    end
end
