...
for i = 1:size(IDsArray, 1)
    ...
    if ~isempty(ia)
        ...
        for j = 1:size(ia, 1)
            ...
            switch typeID
                case 'InChI'
                    inchiLayersDetail = getInchiData(IDsArray{i, ia(j)});
                    layersOfDataInChI(j, 1) = inchiLayersDetail.layers;
                ...
                case 'SMILES'
                    ...
                    [~, result] = system(command);
                    result = split(result);
                    inchiLayersDetail = getInchiData(result{contains(result, 'InChI=1S')});
                    layersOfDataInChI(j, 1) = inchiLayersDetail.layers;
                    idsScore(j) = idsScore(j) + 4 + layersOfDataInChI(j, 1); % scale in formula comparison
                    reportComparison.(['met_' groupedIDs.metNames{i}]).layersOfDataInChI(j, 1) = ...
                        layersOfDataInChI(j, 1);
