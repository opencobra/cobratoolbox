...
% 2 reactions.* The function bio_draw_by_rxn has 7 input arguments: (1) model – stands 
...
% and can be unset. The function bio_draw_by_rxn has 2 outputs: (1) involvedMets – 
...
%% _Application of command_ _*bio_draw_by_rxn *_
% _*bio_draw_by_rxn _*can be performed using *option* *A* in case COBRA model optimization 
...
% 
% 
% *Fig. 2.* The scenarios of an application of the command _bio_draw_by_rxn_.
% 
% 
% 
...
% available for the command _bio_draw_by_rxn_.
...
% The command _bio_draw_by_rxn_ has several input arguments – _model, rxns_,_ drawMap_, 
% _direction_, _initialMet_, _excludeMets_, and_ flux_. The last 5 arguments are 
% optional; it means that the algorithm of the command _bio_draw_by_rxn_ uses default 
...
% TODO: These reactions are not in the e coli core model. I don't know
% which model was used as the test model
...
% of the command _bio_draw_by_rxn._
...
% command _draw_by_met_ is calling out the command _bio_draw_by_rxn _and passing the 
...
% v1.3 because the command _draw_by_met_ is calling out the command _bio_draw_by_rxn_ 
...
% The command _bio_draw_by_rxn _has two output vectors in the result: _involvedMets 
% _and _deadEnds_.
% 
...
[directionRxns, involvedMets, deadEnds] = draw_by_met(model, {'etoh[c]'}, 'true', 2) %EMPTY CELLS
%% 
% 
% 
...
[directionRxns, involvedMets, deadEnds] = draw_by_met(model, {'etoh[c]'}) %EMPTY CELLS
%% 
...
% are used for the function [involvedMets, deadEnds] = bio_draw_by_rxn(model, rxns, 
...
% for the function [involvedMets, deadEnds] = bio_draw_by_rxn(model, rxns, drawMap, 
...
% used for the function [involvedMets, deadEnds] = bio_draw_by_rxn(model, rxns, drawMap, 
% direction, initialMet, excludeMets, flux), the rest will take default values. 
...
[directionRxns,involvedMets,deadEnds] = draw_by_met(model,{'etoh[c]'},'false',2) % EMPTY CELLS
%% 
