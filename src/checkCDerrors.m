function [rxn,met,rev,form] = checkCDerrors(map,model)
    ...
    % avoid to obtain subsystems as metabolites (therefore obtain just
    % metabolites) 
    ...
    % commonMets = intersect(map.specName, model.mets)
    % extraMetsMap = setdiff(map.specName, model.mets)
    % extraMetsModel = setdiff(model.mets,map.specName)

    %% check reversibility 

    % reversibility in the map 
    ...
    %%comparations between Recon3 and Cell Designer
    ...
    [Wrong_table,Absent_model_table,Absent_map_table,Duplicate_table] = Compare_model_map_formulas(model, map);

    form = struct('wrong_reaction_formula',{Wrong_table}, 'reactions_in_map_not_in_model',{Absent_model_table},'reactions_in_model_not_in_map',{Absent_map_table}, 'duplicated_reactions',{Duplicate_table});
