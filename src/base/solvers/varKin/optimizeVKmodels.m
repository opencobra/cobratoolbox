...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% optimizeVKmodels is a function for finding a solution of the nonlinear 
% system
%                h(x) = f(x) = 0, x in R^m,                (I)
% or
%                h(x) = (f(x)^T,l(x)^T)^T = 0, x in R^m,   (II)
% where 
%                f(x) = [F-R, R-F]*exp(k+[F,R]'*x),         
%                l(x) = L*exp(x)-l0,
...
% If sonver fiels is empty, the code will use LMTR as the default.
%
% INPUT:
%
