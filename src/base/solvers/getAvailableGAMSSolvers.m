function [table, numTable, problemType, solvers] = getAvailableGAMSSolvers
% This function return the GAMS solvers which are available in your system
% according to the license.
%
% USAGE:
%
%    [table, numTable, problemType, solvers] = getAvailableGAMSSolvers
%
% OUTPUTS:
%    table:          Cell array - matrix summarizing which problem can be
%                    solved by which solver. A "yes" in position (`i,j`) means
%                    that the GAMS solver "j" is available in your system to
%                    solve a problem of type "i". Please note that headers
%                    are included in this table, so the first column and
%                    the first row have labels for solvers and problem
%                    types, respectively.
%    numTable:       Double matrix summarizing which problem can be
%                    solved by which solver. A "1" in position (`i,j`) means
%                    that the GAMS solver "j" is available in your system to
%                    solve a problem of type "i"
%    problemType:    Cell array of strings - list of problem types that can be solved
%                    in GAMS. Example: LP, MIP, etc
%
%    solvers:        Cell array of strings - list of solvers available in GAMS
%
% .. Author: - Sebastian Mendoza, 30/05/2017. snmendoz@uc.cl

gamsPath = which('gams'); % verify that gams is installed
if isempty(gamsPath)
    error('GAMS is not installed or GAMS path has not been added to MATLAB path.');
end

%verify that licememo.gms in the path of the system
licememoFullPath = which('licememo.gms');
if isempty(licememoFullPath)
    error('licememo.gms in not in MATLAB path.');
end

%copy licememo.gms to the current folder
current = pwd;
if ~strcmp(regexprep(licememoFullPath, {'\\licememo.gms', '/licememo.gms'}, {'', ''}), current)
    copyfile(licememoFullPath);
end

%run licememo.gms
run = system('gams licememo');

%if licememo.gms ran correctly.
if run == 0
    % read the content of the file generated by licememo.gms to find the
    % GAMS solvers available
    fileID = fopen('licememo.txt', 'r');
    table = cell(2, 2);
    numTable = zeros(120, 30);
    tline = fgetl(fileID);
    solvers = {};
    while ischar(tline)
        if strfind(tline,'Solver/Modeltype Matrix:')
            tline = fgetl(fileID); if isempty(tline); tline = fgetl(fileID); end;
            problemType = {};
            aux = regexp(tline, ' ', 'split');
            for i = 1:length(aux)
                if ~isempty(aux{i})
                    problemType(end + 1) = aux(i);
                end
            end
            nProblemType = length(problemType);

            nSolvers = 0;
            tline = fgetl(fileID);
            while ischar(tline)
                solver = regexprep(tline(1:12), ' ', '');
                nSolvers = nSolvers + 1;
                solvers{nSolvers} = solver;
                for i = 1:length(problemType)
                    if length(tline) < 12 + 7 * (i - 1)
                        continue;
                    elseif length(tline)>12 + 7 * (i - 1) && length(tline)< 12 + 7 * (i) && ~isempty(regexprep(tline(12 + 7 * (i - 1):end), ' ', ''))
                        table{nSolvers,i} = 'yes';
                        numTable(nSolvers,i) = 1;
                        continue;
                    elseif ~isempty(regexprep(tline(12 + 7 * (i - 1):12 + 7 * i), ' ', ''))
                        table{nSolvers,i} = 'yes';
                        numTable(nSolvers,i) = 1;
                    end
                end
                tline = fgetl(fileID);
            end
            break;
        end
        tline = fgetl(fileID);
    end

    numTable = numTable(1:nSolvers, 1:nProblemType);
    table = [{''}, problemType; [solvers', table]];
    fclose(fileID);
    %delete autogenerated file
    delete('licememo.txt')
else

    table ={};
    numTable = 0;
    problemType = {};
    solvers = {};
end
%delete autogenerated file and licememo.gms
delete('licememo.gms')
delete('licememo.lst')

end
