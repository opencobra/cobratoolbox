...
%                      * .comps - (optional) compartment symbols (default:
%                        `model.comps` if present, otherwise `getDefaultCompartments`)
%                      * .compNames - (optional) compartment names
%                        corresponding to `.comps`
%                      * .rules - (optional) used to (re)build `.grRules`
%                        via `creategrRulesField` when present
%                      * .osenseStr - (optional) objective sense
%                        (`'max'`/`'min'`); if `'min'`, `.c` is negated for export
%                      * .c - objective coefficients, negated in place when
%                        `.osenseStr` is `'min'`
%                      * .subSystems - (optional) subsystem assignment(s) per
%                        reaction; merged with `;` if provided as nested cells
%                      * .formulas - written by this function: reaction
%                        formulas generated by `printRxnFormula`
%                      * .metCompartments - written by this function:
%                        compartment name per metabolite
