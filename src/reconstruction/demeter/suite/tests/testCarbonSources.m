function [TruePositives, FalseNegatives] = testCarbonSources(model, microbeID, biomassReaction, inputDataFolder)
...
if isempty(mInd)
    ...
    % perform FVA to identify flux metabolites
    % set BOF
...
%     model = changeRxnBounds(model, biomassReaction, 1e-3, 'l');
    ...
    if ~isempty(rxns)
        ...
        if isempty(rxnsInModel)
            ...
            % which carbon sources should be taken up according to in vitro data
            csData = find(table2array(carbonSourcesTable(mInd, 2:end)) == 1);
            
            % check all exchanges corresponding to each carbon source
            TruePositives = {};  % true positives (flux in vitro and in silico)
            FalseNegatives = {};  % false negatives (flux in vitro not in silico)
            for i = 1:length(csData)
                tableData = table2array(carbonSourcesExchanges(csData(i), 2:end));
...
if ~isempty(FalseNegatives)
    ...
    for i = 1:length(FalseNegatives)
        ...
        warning(['Microbe "' microbeID, '" cannot take up carbon source "', FalseNegatives{i}, '".'])
    end
end
