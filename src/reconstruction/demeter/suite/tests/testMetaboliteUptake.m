function [TruePositives, FalseNegatives] = testMetaboliteUptake(model, microbeID, biomassReaction)
...
uptakeTable = readtable('uptakeTable.txt', 'Delimiter', '\t');
...
if ~isempty(FalseNegatives)
    ...
    for i = 1:length(FalseNegatives)
        ...
        warning(['Microbe "' microbeID, '" cannot secrete uptake product "', FalseNegatives{i}, '".'])
    end
end
