function [model] = targetedGapFilling(model,biomassReaction,database)
...
if FBA.f < tol
    ...
    if FBA.f < tol
        ...
        for i=1:length(model.mets)
            ...
            FBA = optimizeCbModel(modelSink);
            if FBA.f > tol
                growthEnablingMets{geCnt,1}=model.mets{i};
                geCnt=geCnt+1;
            end
        end
        ...
        if ~isempty(find(ismember(growthEnablingMets,'gam6p[c]'))) && ~isempty(find(ismember(model.rxns,'HEX1')))
            ...
            for i=1:length(rxns)
                if isempty(find(ismember(model.rxns, rxns{i})))
                    model = addReaction(model, [rxns{i},'_TGF'], database.reactions{find(ismember(database.reactions(:, 1), rxns{i})), 3});
