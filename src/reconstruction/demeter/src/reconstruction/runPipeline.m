function [refinedFolder, translDraftsFolder] = runPipeline(draftFolder, varargin)
...
% USAGE:
%
%    runPipeline(draftFolder, varargin)
%
% REQUIRED INPUTS
...
% translDraftsFolder        Folder with draft COBRA models with translated
%                           nomenclature and stored as mat files
% infoFilePath              File with information on reconstructions to
%                           refine
% inputDataFolder           Folder with experimental data and database files
%                           to load (default: semiautomaticrefinement/input)
% getComparativeGenomics    Boolean indicating whether PubSEED spreadsheets 
%                           with information on the reconstructed strains 
%                           are available and should be used (default: false)
...
parser.addParameter('refinedFolder', '', @ischar);
parser.addParameter('translDraftsFolder', '', @ischar);
parser.addParameter('summaryFolder', '', @ischar);
...
translDraftsFolder = parser.Results.translDraftsFolder;
...
if isempty(inputDataFolder)
    ...
    mkdir('inputFiles')
    inputDataFolder = [pwd filesep 'inputFiles'];
end
...
if isempty(infoFilePath)
    ...
    writetable(cell2table(infoFile),[inputDataFolder filesep 'infoFile.txt'],'FileType','text','WriteVariableNames',false,'Delimiter','tab');
    infoFilePath = [inputDataFolder filesep 'infoFile.txt'];
end

% determine some inputs if not specified
if isempty(refinedFolder)
    % create a folder in the current path
    mkdir('refinedReconstructions')
    refinedFolder = [pwd filesep 'refinedReconstructions'];
end

if isempty(translDraftsFolder)
    % create a folder in the current path
    mkdir('translatedDraftReconstructions')
    translDraftsFolder = [pwd filesep 'translatedDraftReconstructions'];
end

if isempty(summaryFolder)
    % create a folder in the current path
    mkdir(['refinementSummary_' reconVersion])
    summaryFolder = [pwd filesep ['refinementSummary_' reconVersion]];
end
...
for i=1:steps:length(models)
    ...
    for j=i:i+endPnt
        if j <= length(models)
            ...
            save([translDraftsFolder filesep outputFileNamesTmp{j,1}],'model');
...
if createSBML
    createSBMLFiles(inputFolder, sbmlFolder)
end
