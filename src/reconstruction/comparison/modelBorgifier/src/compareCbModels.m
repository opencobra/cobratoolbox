...
%
function [Cmodel, Tmodel, score, Stats] = compareCbModels(Cmodel, Tmodel)
% compareCbModels Calculate similarity of two Cb models. These models must
% be in a COBRA-like format.
%
% USAGE:
...
%               Size = MxNxF
%    Stats      Structure of statsistics. Including
%    scoreTotal    MxN matrix of normalized (0 to 1) scores with M equal 
%                  to the number of reactions in Cmodel and N equal to the 
%                  number of reactions in Tmodel.
%
% CALLS:
%    addComparisonInfo:     Subfunction. 
%    compareMetInfo:        Subfunction.
%    optimalScores
%    mapMatch
%    buildRxnEquations
%    fixNames
...
% Reversibility Match. Uses logical indexing. 
tic
for iM = 1:length(modelNames)
    ...
    for i = compCrxns
        if Cmodel.rev(i)
            ...
            score(i, ~revLog, ScoreVal.revM(2)) = ...
                score(i, ~revLog, ScoreVal.revM(2)) + ScoreVal.revM(1) ;
        else
            score(i, ~revLog, ScoreVal.revH(2)) = ...
                score(i, ~revLog, ScoreVal.revH(2)) + ScoreVal.revH(1) ;
            score(i, revLog, ScoreVal.revM(2)) = ...
...
fprintf('Reversibility match time = %d.\n', toc)
