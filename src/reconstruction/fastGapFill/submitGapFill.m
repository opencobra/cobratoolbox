function [AddedRxns] = submitGapFill(swift, modelFile, dbFile, dictionaryFile, prepareFGFResults, weightsPerRxnFile, forceRerun, epsilon, blackList, listCompartments)
% A test function for `prepareFastGapFill`, `swiftGapFill`, and
...
%
% This function performs three runs of `fastGapFill` or `swiftGapFill on the 
% same prepared model, with weights as specified in the 'Define Runs' section 
% of the code.  The 'runs' variable can be appended to or changed to enable 
% the running of `fastGapFill` or `swiftGapFill` for different combinations 
...
% in running `fastGapFill` or `swiftGapFill` so should be specified as empty 
% if running without a weight file.
%
...
if ~exist('swift','var') || isempty(swift)
    swift=false;
end
...
if swift
    Stats{cnt,1} = 'Time swiftGapFill';cnt = cnt+1;
else
...
try
    ...
    fprintf('Using the preloaded variables\n');
end
...
for i = 1:length(runs)
    ...
    tic;
    if swift
        [~, AddedRxns] = swiftGapFill(consistMatricesSUX,1e-10,weights,weightsPerReaction);
    else
