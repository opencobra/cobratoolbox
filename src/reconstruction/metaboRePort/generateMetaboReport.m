...
% This function generates the metaboReport: an HTML report summarizing
% model properties and scores for one or more metabolic models, using
% `reportTemplate.html` as a template.
%
% USAGE:
%
...
%
% INPUTS:
%    modelProperties:    Structure containing, for each model, the
%                        properties generated by `generateMetaboScore`
%                        (one field per model, each holding a `modelProp2`
%                        substructure)
%    reportDir:          Directory in which the generated
%                        `modelreport_<ModelName>.html` file(s) are saved
%
% OPTIONAL INPUTS:
%    orgNames:           Cell array of organism/model display names, one
%                        per field of `modelProperties` (default: falls
%                        back to the corresponding field name)
%
% .. Author: - Ines Thiele, 2022
