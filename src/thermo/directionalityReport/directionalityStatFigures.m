function directionalityStatFigures(model,directions,figures)
...
directions.fwdReconBool = (model.qualDir == 1);
directions.revReconBool = (model.qualDir == -1);
directions.reversibleReconBool = (model.qualDir == 0);
...
if exist('figures','var')
    ...
    h=pie(data,{'Forward','Reverse','Reversible'});
    ...
    % Lemmer: have a look into directions.fwdReconThermoBool,
    % directions.revReconThermoBool and
    % directions.reversibleReconThermoBool
    % why are they 0?
...
%     {'\Delta_{r}G^{\primem} > 0',int2str(data(7))}});
    ph=pie(gca,data,{'\Delta_{r}G^{\primem} < 0',...
    '\Delta_{r}G^{\primem}_{est} < 0',...
    '0 << \Delta_{r}G^{\primem}_{est} > 0',...
    '0 << \Delta_{r}G^{\primem}_{est} >> 0',...
    '0 < \Delta_{r}G^{\primem}_{est} >> 0',...
    '\Delta_{r}G^{\primem}_{est} > 0',...
    '\Delta_{r}G^{\primem} > 0'});
