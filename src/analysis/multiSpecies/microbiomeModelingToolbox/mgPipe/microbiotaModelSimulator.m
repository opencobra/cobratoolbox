...
if skipSim==1
    ...
    for s=1:steps:length(sampNames)
        ...
        for k=s:s+endPnt
            ...
            if ~isempty(netProduction{k})
                vals=netProduction{k}(find(~cellfun(@isempty,(netProduction{k}(:,2)))),2);
                if abs(sum(cell2mat(vals)))> 0.1
                    doSim=0;
                end
            end
            if doSim==1 || repeatSim==1
                ...
                if solution_allOpen.stat==0
                    ...
                    if rDiet==1 && computeProfiles
                        ...
                        minFluxDiet = minFlux;
                        maxFluxDiet = maxFlux;
                        netProdTmp{1,k}=exchanges;
                        netUptTmp{1,k}=exchanges;
                        for i =1:length(FecalRxn)
                            [truefalse, index] = ismember(FecalRxn(i), exchanges);
                            netProdTmp1{k}{index,2} = minFluxDiet(i,1);
                            netProdTmp1{k}{index,3} = maxFluxFecal(i,1);
                            netUptTmp1{k}{index,2} = maxFluxDiet(i,1);
                            netUptTmp1{k}{index,3} = minFluxFecal(i,1);
                        end
                    end
                    if rDiet==1 && saveConstrModels
                        microbiota_model=model;
                        mkdir([resPath filesep 'Rich'])
                        parsave([resPath filesep 'Rich' filesep 'microbiota_model_' sampleID '.mat'],'microbiota_model')
                    end
                    ...
                    if solution_sDiet.stat==0
                        ...
                        if computeProfiles
                            ...
                            netProdTmp2{k}=exchanges;
                            netUptTmp2{k}=exchanges;
                            for i =1:length(FecalRxn)
                                [truefalse, index] = ismember(FecalRxn(i), exchanges);
                                netProdTmp2{k}{index,2}=minFlux(i,1);
                                netProdTmp2{k}{index,3}=maxFlux(i,1);
                                netUptTmp2{k}{index,2}=smi2(i,1);
                                netUptTmp2{k}{index,3}=sma2(i,1);
                            end
                        end
                        ...
                        if pDiet==1
                            ...
                            presolTmp{k}(3)=solution_pdiet.obj;
                            if isnan(solution_pdiet.obj)
                                warning('Presolve detected one or more infeasible models. Please check InFesMat object !')
                                inFesMatTmp{k}(3) = model.name;
                            else
                                ...
                                if computeProfiles
                                    ...
                                    maxFlux=sma;
                                    minFlux=smi;
                                    netProdTmp3{k}=exchanges;
                                    for i = 1:length(FecalRxn)
                                        [truefalse, index] = ismember(FecalRxn(i), exchanges);
                                        netProdTmp3{k}{index,2}=minFlux(i,1);
                                        netProdTmp3{k}{index,3}=maxFlux(i,1);
                                        netUptTmp3{k}{index,2}=smi2(i,1);
                                        netUptTmp3{k}{index,3}=sma2(i,1);
                                    end
                                end
