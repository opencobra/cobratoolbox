function [ReactionAbundance,TaxonomyInfo] = calculateReactionAbundance(abundancePath, modelPath, taxonomyPath, rxnsList, numWorkers, byTaxon)
...
% USAGE
%
%    [ReactionAbundance,TaxonomyInfo] = calculateReactionAbundance(abundancePath, modelPath, taxonomyPath, rxnsList, numWorkers, byTaxon)
%
% INPUTS:
...
%    byTaxon                 Flag whether the reaction abundances on taxon
%                            level should be computed (default: true)
%
% OUTPUT:
...
if exist('taxonomyPath','var') && ~isempty(taxonomyPath)
    taxonomy = readtable(taxonomyPath, 'ReadVariableNames', false);
...
for i = 1:length(fNames)
    ...
    if ~isempty(delArray)
     ...
     ReactionAbundance.(fNames{i}){1,1}='Sample';
end
