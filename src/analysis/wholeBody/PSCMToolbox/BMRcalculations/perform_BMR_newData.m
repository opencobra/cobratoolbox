% This script repeats the simulation described in Thiele et al., "Personalized whole-body models integrate metabolism, physiology, and the gut microbiome", Method section 3.9.2 Validation of the parameters in an independent data set.
% The input data come from  L. L. Loureiro et al., Basal Metabolic Rate of Adolescent Modern Pentathlon Athletes: Agreement between Indirect Calorimetry and Predictive Equations and the Correlation with Body Parameters. PLoS One, 10(11):e0142859 (2015).

global useSolveCobraLPCPLEX

clear Energy_kcal_femaleBMR2 %avoid adding another column to this variable
...
cnt = 1;
for y =2 %1 : length(sensi_analysis)
    femaleBMR = female;
    clear female
    ...
    for z =1%1: length(sensi_analysis)
        ...
        InputDataAll =  {
            'sex' ''	'male'		'female'		'female'		'female'		'female'		'female'		'male'		'female'		'male'		'male'		'male'		'female'		'male'		'male'		'male'		'male'		'male'		'female'		'female'		'female'		'male'		'male'		'male'		'female'		'male'		'male'		'male'		'male'
        ...
        for i = 3 :  size(InputDataAll,2) %
            ...
            if strcmp(sex,'female')
                ...
                if useSolveCobraLPCPLEX
                    ...
                    femaleBMR2.osense = -1;
                    [solution] = computeMin2Norm_HH(femaleBMR2,'tomlab_cplex');
                else
                    femaleBMR2.c(:)=0;
                    param.minNorm = 1e-6;
                    femaleBMR2.osenseStr='min';
        ...
        %save([resultsPath 'Results_BMR_newData'])
    end
end
for i = 3 : size(InputDataAll,2)
...
clearvars -except computeResults useSolveCobraLPCPLEX InputDataAll Energy_kcal_femaleBMR2 BMR_female_20 male Energy_kcal_femaleBMR2_origStatText
            ...
            for i = 3 :  size(InputDataAll,2) %
                ...
                if strcmp(sex,'male')
                    ...
                    if useSolveCobraLPCPLEX
                        ...
                        maleBMR2.osenseStr = 'max';
                        maleBMR2.osense = -1;
                        [solution] = computeMin2Norm_HH(maleBMR2);
                    else
                        maleBMR2.c(:)=0;
                        param.minNorm=1e-6;
                        maleBMR2.osenseStr = 'min';
                        solution = optimizeWBModel(maleBMR2,param);
                    end
                    if solution.stat == 1 || solution.stat == 3
                        F = max(-maleBMR2.S,0);
                        R = max(maleBMR2.S,0);
                        ...
                        Energy_kJ_maleBMR2(i,cnt) = NaN;
                    end
                    Energy_kcal_maleBMR2_origStatText{i,cnt} = solution.origStatText;
                end
            end
            cnt = cnt +1;
            %save Results_sensi2_2018_12_14
...
clearvars -except computeResults useSolveCobraLPCPLEX InputDataAll male BMR_male_20 Energy_kcal_maleBMR2 Energy_kcal_maleBMR2_origStatText female BMR_female_20 Energy_kcal_femaleBMR2 Energy_kcal_femaleBMR2_origStatText 