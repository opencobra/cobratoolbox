...
for i = a : z %length(listing)
    ...
    % convert to germfree
        modelHM.lb(find(ismember(modelHM.rxns,'Excretion_EX_microbiota_LI_biomass[fe]')))=0; %
        modelHM.ub(find(ismember(modelHM.rxns,'Excretion_EX_microbiota_LI_biomass[fe]')))=0; %
    ...
    if ~isempty(ResultsATP_Fat)
        ...
        save(strcat('Results_GF_',file));
    else
        save(strcat('Results_GF_Inf_',file));
    end
end
