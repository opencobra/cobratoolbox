...
for i = a : z %length(listing)
    ...
    modelHM.ub(strmatch('EX_pcs[u]',modelHM.rxns))=1000000;
    ...
    [SizeMatrix(i,1),SizeMatrix(i,2)] = size(modelHM.A);
    SizeMatrix(i,3) =  SizeMatrix(i,1)-length(find(~cellfun(@isempty,strfind(modelHM.mets,'slack'))));
    NumMicrobes(i,1) = length(modelHM.A(:,(strmatch('communityBiomass',modelHM.rxns))))-1;
    
    clear Organ* Obje* Indivi* Number*  k  gender  d ID Body* BloodF* sol ResultsATP_Fat_S*  bloodF*
    
    save(strcat('Results_HMPNew2',file));
    %     else
    %         save(strcat('Results_HMPNew2_Inf_',file));
    %     end
    %
    %
    %% germfree:
