function [L, M, moietyFormulae, moieties2mets, moieties2vectors, atoms2moieties, mtrans2rxns, atrans2mtrans,mbool,rbool,V,E,C] = identifyConservedMoieties(model, ATN, method, sanityChecks)
...
% USAGE:
%
%    [L, M, moietyFormulae, moieties2mets, moieties2vectors, atoms2moieties, mtrans2rxns, atrans2mtrans] = identifyConservedMoieties(model, ATN)
%
% INPUTS:
...
[nAtoms, nAtransInstances] = size(A);
...
switch method
    case 'isomorphism'
        ...
        %an atom transition that occurs in a reaction is an atom
        %transition instance, and since identical atom transition instances
        %can happen in a more than one reaction, we only need a representative 
        %atom transition
        [~,uniqueAtomTransitions,~] = unique((F+R)','rows');
        nAtrans = length(uniqueAtomTransitions);
        
        %unique columns of the atom transition graph
        %A = A(:,uniqueAtomTransitions);
        ...
        EdgeTable = table(headTail(uniqueAtomTransitions,:),num2cell((1:nAtrans)'),ATN.rxns(uniqueAtomTransitions),'VariableNames',{'EndNodes','AtomTransIndex','FirstRxn'});
        ...
        for i = 1:nComps
            try
                ...
                Edges.firstRxns = deidentifiedSubgraphs{i,1}.Edges.FirstRxn(ia);
                deidentifiedSubgraphs{i,1}=graph(Edges);
            end
        end
        
        %map of conserved moieties to components of atom transition graph
        C = false(rowRankDeficiencyN,nComps);
        ...
        %remove zero rows
        C = C(any(C,2),:);
        %define the number of isomorphism classes
        nIsomorphismClasses = size(C,1);
        %save appropriate
        nVertFirstSubgraph=nVertFirstSubgraph(1:nIsomorphismClasses,1);
...
%         %map conserved moiety instances to atoms in atom transition graph
%         D = false(sum(nVertFirstSubgraph),nAtoms);
%         for i = 1:nIsomorphismClasses
%             subgraphs{i,1}.Nodes.AtomIndex
%         end
    ...
    case 'leftNull'
        ...
        nVectors = size(L, 2);
        moietyFormulae = cell(nVectors, 1);
        for i = 1:nVectors
            f = tabulate(compElements(xj == i)); % elements in moiety i
            f([f{:, 2}]' == 1, 2) = {''};
            f = f(:, 1:2)';
            moietyFormulae{i} = sprintf('%s%d',f{:});
        end
...
atrans2mtrans = zeros(nAtransInstances,1);
