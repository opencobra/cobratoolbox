function modelConstraint = physiologicalConstraintsHMDBbasedTEMP(model,IndividualParameters, ExclList, Type, InputData, Biofluid, setDefault,ExclMet,ExclMetAbbr)
...
if ~exist('ExclMet','var')
    ExclMet =0; % no further excluded metabolite
    ExclMetAbbr = '';
end
if strcmp(Type,'HMDB')
    % BLOOD
...
for i = 1 : length(OrgansListExt)
    ...
    if ~isempty(tmp) && tmp>0
        if strcmp(IndividualParameters.sex,'male') % use first col
    ...
    for i = 1 : length(OrgansListExt)
        ...
        if ~isempty(ExR)
            for j = 1 : length(ExR)
                ...
                % this does not ensure it is coming from blood though? is
                % there a reason for the '||' OR?- Anna Sheehy changed to
                % '&&' 27.11.2024
                if (length(strfind(modelConstraint.rxns{ExR(j)},'[bc]'))>0 || length(strfind(modelConstraint.rxns{ExR(j)},'[CSF]upt'))>0) ...
                    ...
                    %% allow to exclude more metabolites from being constrained
                    % not nice programming but I had problems with other
                    % versions
                    goOn = 1;
                    if ExclMet ==1 %length(strfind(modelConstraint.rxns{ExR(j)},ExclMetAbbr))==0
                        for q =1 : length(ExclMetAbbr)
                            if length(strfind(modelConstraint.rxns{ExR(j)},ExclMetAbbr{q}))>0 % metabolite is found
                                goOn = 0;
                            end
                        end
                    end
                    if goOn ==1
                        ...
                        % ANNA's temp edit for this line:
                        %MCon = [];
                        if strcmp(Type,'direct')
                            MCon = (metConcDataBc{1, 2} + metConcDataBc{1, 3}) / 2;
                        else 
                            MCon = [];
                        end
                        ...
                        if ~isempty(ExM)
                            ...
                            if ~isempty(strmatch(OrgansListExt(i),'Kidney','exact')) && length(strfind(modelConstraint.rxns{ExR(j)},'[bcK]'))==0
                                ...
                                % get maximal concentration for metabolite
                                X = find(ismember(strcat(metConcDataBc{:,VMHIDCol}),ExM));
                              if isempty(X) %co2 and o2 are now getting in straight from rbc
                                X = find(ismember(strcat('RBC_',metConcDataBc{:,VMHIDCol}),ExM));
                              else
            ...
            if length(strfind(modelConstraint.rxns{ExR(j)},'[u]'))>0 && length(strfind(modelConstraint.rxns{ExR(j)},'_o2(e)'))==0 ... %no oxygen constraint
                ...
                if ~isempty(ExM)
                    ...
                    if ~isempty(MConMin) && ~isempty(MConMax)
                        MSecrRateUrLB = (MConMin/1000)*MConDefaultUrCreatinineMin*UrFlowRate/1000; % in mmol/day/person
                        % upper bound based on max concentration
                        MSecrRateUrUB = (MConMax/1000)*MConDefaultUrCreatinineMax*UrFlowRate/1000; % in mmol/day/person
                        R = { 'EX_na1[u]'
                            'EX_cl[u]'
                            'EX_k[u]'
                            'EX_ca2[u]'
...
if ~strcmp(Type,'direct')
    ...
    if 0
     ...
     % gas = 22.4 L standard temperature and pressure, dry (STPD) conditions. 
        met = 0.07; %ml o2/min/100g
        met = (met * 1000/22.4) *60 *24; % umol o2/ day/100g;
         muscle_weight = (IndividualParameters.OrgansWeights(find(ismember(IndividualParameters.OrgansWeights(:,1),'Muscle')),2));
         if ischar(muscle_weight)
             muscle_weight =str2num(char(muscle_weight{1})) ;
         elseif iscell(muscle_weight)
             try
                     muscle_weight =str2num((muscle_weight{1})) ;
             end
             try
                 muscle_weight =muscle_weight{1} ;
             end
         else
             muscle_weight =muscle_weight{1} ;
         end
         met =( met*muscle_weight/100)/1000; % mmol o2/day/person
           if modelConstraint.lb(find(ismember(modelConstraint.rxns, 'Muscle_EX_o2(e)_[bc]')))<-met*1.2 && modelConstraint.ub(find(ismember(modelConstraint.rxns, 'RBC_EX_glc_D(e)_[bc]')))>=-met*1.2
            modelConstraint = changeRxnBounds(modelConstraint,'Muscle_EX_o2(e)_[bc]',-met*0.8,'u');
            modelConstraint = changeRxnBounds(modelConstraint,'Muscle_EX_o2(e)_[bc]',-met*1.2,'l');
        elseif modelConstraint.lb(find(ismember(modelConstraint.rxns, 'Muscle_EX_o2(e)_[bc]')))> modelConstraint.ub(find(ismember(modelConstraint.rxns, 'RBC_EX_glc_D(e)_[bc]')))
            modelConstraint.ub(find(ismember(modelConstraint.rxns, 'Muscle_EX_o2(e)_[bc]')))=0; %reset earlier constraints if lb>ub
           end
    end
    if 1
        if 1
        ...
        %
        if 1
            brain_weight = (IndividualParameters.OrgansWeights(find(ismember(IndividualParameters.OrgansWeights(:,1),'Brain')),2));           
         if ischar(brain_weight)
             brain_weight =str2num(char(brain_weight{1})) ;
         elseif iscell(brain_weight)
             try
             brain_weight =str2num((brain_weight{1})) ;
             end
             try
