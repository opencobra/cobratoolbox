...
if nargin<4
    ...
    % id = find(Flux==0); %find rxn without flux 
    % rx = model.rxns(id,1)
    % index2 = find(ismember(map.rxnName,rx)); % find non-zeros elements 
    % map2.rxnColor(index2,1) = {Colors('Grey')} % color all reactions without flux
