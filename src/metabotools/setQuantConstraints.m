...
for j = 1:length(samples)
    ...
    j
    %   load([path '\model.mat']); %% uncomment for Recon1
    ExtraExchAdded = 0;
    
    
    FILENAME = char(samples(j,1));
    load([path FILENAME '.mat'])%, 'uptake_value', 'secr_value' , 'uptake' ,'secretion', 'cell_line' );
    ...
    save([path 'setQuantConstraints.mat'], '-v7.3');
end
