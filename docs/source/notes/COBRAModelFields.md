...
|`model.rules`| `n x 1` | Column Cell Array of Strings | "GPR rules in evaluateable format for each reaction ( e.g. ""x(1) &#124; x(2) & x(3)"", would indicate the first gene or both the second and third gene are necessary for the respective reaction to carry flux" model.rules should only be used for internal processing within a function, the primary storage of GPR rules is model.grRules because it is based on the string identifying a gene, rather than the positon in a cell arrray.| 
