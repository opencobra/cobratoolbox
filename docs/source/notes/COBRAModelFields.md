...
|`model.metNames`| `m x 1` | Column Cell Array of Strings | Full names of each corresponding genes. | 
|`model.metNotes`| `m x 1` | Column Cell Array of Strings | Entrez IDs of genes | 
|`model.metHMDBID`| `m x 1` | Column Cell Array of Strings | A string representation of the GPR rules defined in a readable format. | 
|`model.metInChIString`| `m x 1` | Column Vector of Double | The charge of the respective metabolite (NaN if unknown) | 
|`model.metKEGGID`| `m x 1` | Column Cell Array of Strings | Formula for each metabolite in the ChEBI format. | 
|`model.metChEBIID`| `m x 1` | Column Cell Array of Strings | Elemental formula for each metabolite. | 
|`model.metPubChemID`| `m x 1` | Column Cell Array of Strings | Formula for each metabolite in the HMDB format. | 
...
|`model.rxnConfidenceScores`| `n x 1` | Struct | The version of the model, Store FBC versions in the COBRA structure | 
|`model.rxnNames`| `n x 1` | Column Cell Array of Strings | Full Name for each Protein | 
|`model.rxnNotes`| `n x 1` | Column Cell Array of Strings | Proteins associated with each reaction. | 
|`model.rxnECNumbers`| `n x 1` | Column Vector of double | Confidence scores for reaction presence (0-5, with 5 being the highest confidence) | 
