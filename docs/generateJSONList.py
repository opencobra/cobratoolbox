...
def path_to_list(path, folder):
    ...
    for root, directories, filenames in os.walk(path):
        for filename in filenames:
            print os.path.join('source', 'modules', folder, root[path_length+1:], 'index.rst')
            if filename[-2:] == '.m' and os.path.isfile(os.path.join('source', 'modules', folder, 'index.rst')):
                website_url = os.path.join("https://opencobra.github.io/cobratoolbox/stable/modules", folder, "index.html")
                website_url += "?highlight=" + filename[:-2]
