function [definitions,success] = tabcomplete(askAboutBackup,funcName, varargin)
        ...
        if ~exist(bakFilename,'file') && ~ispref(mfilename,'dontBackup')
            ...
            switch getQuestDlg(msg)
                case 'Yes, and create Backup'   % => Yes: backup file
                    ...
                    if ~status          
                        success = 0;
                        warning('YMA:TabComplete:CreateBakup','Failed to create %s not attempting to alter the Tab Completion:\n%s',bakFilename,msg);
                    end
                case 'Yes, without Backup'
                    success = 1;% => No & don't ask again
                case ''
                    success = 0;
