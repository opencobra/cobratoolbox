...
***********************************************************************************************************
*       The objective of this code is to identify the pair of fluxes, say j1 and j2, such
*       that one of the two decreases in value for overproduction. The wild-type flux ranges are stored as parametric
...
*       matrix1 = indexing matrices to store the information    *
*               about the pairs identified                      *
...
$if not set gdxin $set gdxin MtoGLL
$GDXIN %myroot%%gdxin%
...
outer_obj..                                             z =e= sum(j$(can(j) and not constraint_flux(j) ), (-w1(j)-w2(j) ) - (-basemin(j)*y1(j)-basemin(j)*y2(j) ) );
