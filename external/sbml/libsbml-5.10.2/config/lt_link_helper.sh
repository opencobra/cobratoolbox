        ...
        *.o)
        ...
        lib*.la | */lib*.la)
            mkdir -p "${stage_dir}/${libdir}" && \
                eval "/home/rfleming/work/sbg-code/cobratoolbox/external/sbml/libsbml-5.10.2/libtool --mode=install cp \"$1\" \"${stage_dir}/${libdir}\""
...
eval "/home/rfleming/work/sbg-code/cobratoolbox/external/sbml/libsbml-5.10.2/libtool --tag=\"$TAG\" --mode=link \"$LD\" -rpath \"${libdir}\" -inst-prefix-dir \"${stage_dir}\" -module -avoid-version -export-dynamic -shrext "${dllibext}" -o \"${out_dir}/${out_filename}.la\" $args" && \
    eval "/home/rfleming/work/sbg-code/cobratoolbox/external/sbml/libsbml-5.10.2/libtool --mode=install cp \"${out_dir}/${out_filename}.la\" \"${stage_dir}/${libdir}/${out_filename}.la\"" 
