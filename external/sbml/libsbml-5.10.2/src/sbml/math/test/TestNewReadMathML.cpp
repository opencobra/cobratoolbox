...
START_TEST (test_element_csymbol_time_1)
{
  const char* s = wrapMathML
  (
    "<csymbol encoding='text' id='x' "
    "definitionURL='http://www.sbml.org/sbml/symbols/time'> t </csymbol>"
  );
...
START_TEST (test_element_csymbol_delay_4)
{
  ...
  (
    "<apply>"
    "  <csymbol encoding='text' style='d' definitionURL='http://www.sbml.org/sbml/"
    "symbols/delay'> my_delay </csymbol>"
    "  <ci> x </ci>"
    "  <cn> 0.1 </cn>"
    "</apply>\n"
...
START_TEST (test_element_csymbol_delay_5)
{
  const char* s = wrapMathML
  (
    "<apply>"
...
START_TEST (test_element_operator_plus_noargs)
{
  const char* s = wrapMathML
  (
  "<apply> <plus/> </apply>"
...
START_TEST (test_element_operator_plus_onearg)
{
  const char* s = wrapMathML
  (
...
START_TEST (test_element_operator_plus_twoargs)
{
  const char* s = wrapMathML
  (
  "<apply> <plus/> <cn type=\"integer\"> 1 </cn> <cn type=\"integer\"> 2 </cn> </apply>"
  );
...
START_TEST (test_element_operator_plus_threeargs)
{
  const char* s = wrapMathML
  (
  "<apply> <plus/> <cn type=\"integer\"> 1 </cn> <cn type=\"integer\"> 2 </cn> <cn type=\"integer\"> 3 </cn></apply>"
  );
...
START_TEST (test_element_operator_divide)
{
  const char* s = wrapMathML
  (
  "<apply> <divide/> <cn type=\"integer\"> 1 </cn> <cn type=\"integer\"> 2 </cn> </apply>"
  );
  ...
  //fail_unless( N->getChild(3)->getInteger() == 3
...
START_TEST (test_element_operator_times_no_args)
{
  const char* s = wrapMathML
  (
    "<apply> <times/> </apply>"
...
START_TEST (test_element_operator_times_one_arg)
{
  const char* s = wrapMathML
  (
    "<apply> <times/> <ci> x </ci> </apply>"
  );
...
START_TEST (test_element_read_nested)
{
  const char* s = wrapMathML
  (
  "<apply> <divide/> <apply><cos/><cn type=\"integer\"> 1 </cn></apply> <cn type=\"integer\"> 2 </cn> </apply>"
  );
  ...
  fail_unless( N->getType() == AST_DIVIDE);
  fail_unless( N->getNumChildren() == 2);
  fail_unless( N->getChild(0)->getType() == AST_FUNCTION_COS);
  fail_unless( N->getChild(1)->getType() == AST_INTEGER);
  fail_unless( N->getChild(1)->getInteger() == 2);

  ASTNode* c = N->getChild(0);
  fail_unless( c->getType() == AST_FUNCTION_COS);
  fail_unless( c->getNumChildren() == 1);
  fail_unless( c->getChild(0)->getType() == AST_INTEGER);
  fail_unless( c->getChild(0)->getInteger() == 1);
...
START_TEST (test_element_read_nested_1)
{
  const char* s = wrapMathML
  (
  "<apply> <divide/> <apply><cos/><cn type=\"integer\"> 1 </cn></apply> <apply><sin/><cn type=\"integer\"> 2 </cn> </apply></apply>"
  );
  ...
  fail_unless( N->getNumChildren() == 2);
  fail_unless( N->getChild(0)->getType() == AST_FUNCTION_COS);
  fail_unless( N->getChild(1)->getType() == AST_FUNCTION_SIN);
  ...
  ASTNode* c1 = N->getChild(1);
  fail_unless( c1->getType() == AST_FUNCTION_SIN);
  fail_unless( c1->getNumChildren() == 1);
  fail_unless( c1->getChild(0)->getType() == AST_INTEGER);
  fail_unless( c1->getChild(0)->getInteger() == 2);
...
START_TEST (test_element_read_nested_2)
{
  const char* s = wrapMathML
  (
  "<apply> <divide/> <apply><cos/><apply><sin/><cn type=\"integer\"> 1 </cn></apply></apply> <cn type=\"integer\"> 2 </cn> </apply>"
  );
  ...
  fail_unless( c->getType() == AST_FUNCTION_COS);
  fail_unless( c->getNumChildren() == 1);
  fail_unless( c->getChild(0)->getType() == AST_FUNCTION_SIN);
  ...
  fail_unless( c1->getNumChildren() == 1);
  fail_unless( c1->getChild(0)->getType() == AST_INTEGER);
  fail_unless( c1->getChild(0)->getInteger() == 1);
  //F = SBML_formulaToString(N);
  //fail_unless( !strcmp(F, "1 + 2 + 3") );
}
END_TEST
...
START_TEST (test_element_abs)
{
 // const char* s = wrapMathML("<apply><abs/><ci> x </ci></apply>");
  const char* s = wrapMathML("<apply><abs/><cn> 1 </cn></apply>");
...
START_TEST (test_element_arccos)
{
  ...
  const char* s = wrapMathML("<apply><arccos/><cn> 1 </cn></apply>");
...
START_TEST (test_element_arccosh)
{
  ...
  const char* s = wrapMathML("<apply><arccosh/><cn> 1 </cn></apply>");
...
START_TEST (test_element_function_call_2)
{
  ...
  fail_unless (N->getChild(1)->getType() == AST_FUNCTION);
...
START_TEST (test_element_log_3)
{
  const char* s = wrapMathML
  (
    "<apply> <log/> <logbase> <cn type='integer'> 10 </cn> </logbase>"
    "               <ci> x </ci>"
    "</apply>"
  );
...
START_TEST (test_element_piecewise)
{
  ...
  // need to replicate old behaviour
  fail_unless( N->getNumChildren() == 6);

  fail_unless(N->getChild(0)->getType() == AST_FUNCTION_COS);
  fail_unless(N->getChild(1)->getType() == AST_RELATIONAL_LT);
  fail_unless(N->getChild(2)->getType() == AST_REAL);
  fail_unless(N->getChild(3)->getType() == AST_RELATIONAL_EQ);
  fail_unless(N->getChild(4)->getType() == AST_NAME);
  fail_unless(N->getChild(5)->getType() == AST_RELATIONAL_GT);


  ///* the new ast node creates one child for a piece element */
  //fail_unless( N->getNumChildren() == 3);

  //fail_unless(N->getChild(0)->getType() == AST_CONSTRUCTOR_PIECE);
  //fail_unless(N->getChild(0)->getNumChildren() == 2);

  //fail_unless(N->getChild(1)->getType() == AST_CONSTRUCTOR_PIECE);
  //fail_unless(N->getChild(1)->getNumChildren() == 2);

  //fail_unless(N->getChild(2)->getType() == AST_CONSTRUCTOR_PIECE);
  //fail_unless(N->getChild(2)->getNumChildren() == 2);
//F = SBML_formulaToString(N);
...
START_TEST (test_element_piecewise_otherwise)
{
  ...
  //fail_unless(N->getChild(1)->getType() == AST_CONSTRUCTOR_OTHERWISE);
  //fail_unless(N->getChild(1)->getNumChildren() == 1);
  //F = SBML_formulaToString(N);
...
START_TEST (test_element_piecewise_otherwise_1)
{
  const char* s = wrapMathML
  (
    "<piecewise>"
  ...
  fail_unless(N->getChild(0)->getType() == AST_PLUS);
  fail_unless(N->getChild(1)->getType() == AST_CONSTANT_TRUE);
  fail_unless(N->getChild(2)->getType() == AST_NAME);

  F = SBML_formulaToString(N);
  fail_unless( !strcmp(F, "piecewise(1 + 2, true, x)") );
}
END_TEST
...
START_TEST (test_element_root_3)
{
  const char* s = wrapMathML
  (
    "<apply> <root/> <degree> <cn type='integer'> 2 </cn> </degree>"
    "               <ci> a </ci>"
    "</apply>"
  );
...
START_TEST (test_element_root_4)
{
  const char* s = wrapMathML
  (
    "<apply> <root/> <degree> <cn type='integer'> 2 </cn> </degree>"
...
START_TEST (test_element_ci_definitionURL_1)
{
   const char* s = wrapMathML("<ci class=\"ss\" definitionURL=\"foobar\"> x </ci>");
...
START_TEST (test_element_ci_definitionURL_3)
{
   const char* s = wrapMathML("<apply><ci definitionURL=\"foobar\" class=\"sss\"> x </ci><cn> 5 </cn></apply>");
...
START_TEST (test_element_csymbol_avogadro)
{
  ...
  fail_unless( N->getNumChildren() == 0      );
  fail_unless( N->getDefinitionURLString() == "http://www.sbml.org/sbml/symbols/avogadro");
}
END_TEST


START_TEST (test_element_csymbol_avogadro_1)
{
  const char* s = wrapMathML
  (
    "<csymbol encoding='text' "
    "definitionURL='http://www.sbml.org/sbml/symbols/avogadro' style='foo'> NA </csymbol>"
  );
...
create_suite_NewReadMathML ()
{
  ...
  tcase_add_test( tcase, test_element_csymbol_time_1              );
  ...
  tcase_add_test( tcase, test_element_csymbol_delay_5           );
  ...
  tcase_add_test( tcase, test_element_operator_times_no_args            );
  tcase_add_test( tcase, test_element_operator_times_one_arg            );
  tcase_add_test( tcase, test_element_operator_divide             );

  tcase_add_test( tcase, test_element_operator_plus_noargs             );
  tcase_add_test( tcase, test_element_operator_plus_onearg             );
  tcase_add_test( tcase, test_element_operator_plus_twoargs             );
  tcase_add_test( tcase, test_element_operator_plus_threeargs             );

  tcase_add_test( tcase, test_element_read_nested             );
  tcase_add_test( tcase, test_element_read_nested_1             );
  tcase_add_test( tcase, test_element_read_nested_2             );
  ...
  tcase_add_test( tcase, test_element_log_3                     );
  ...
  tcase_add_test( tcase, test_element_piecewise_otherwise_1       );
  ...
  tcase_add_test( tcase, test_element_root_3                    );
  tcase_add_test( tcase, test_element_root_4                    );
  ...
  // tests for attributes on math elements
  tcase_add_test( tcase, test_element_cn_units             );
  tcase_add_test( tcase, test_element_cn_units1             );
  tcase_add_test( tcase, test_element_cn_units2             );
  tcase_add_test( tcase, test_element_cn_units3             );
  tcase_add_test( tcase, test_element_cn_units4             );
  ...
  tcase_add_test( tcase, test_element_ci_definitionURL_3       );
  tcase_add_test( tcase, test_element_csymbol_avogadro              );
  tcase_add_test( tcase, test_element_csymbol_avogadro_1              );
