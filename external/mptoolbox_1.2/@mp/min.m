...
if nargin==1
 ...
 if any(sx==1)
  [out,outp]=minOfVector(x);
  out=xo(outp);
 else
  out=mp(zeros(1,sx(2)),precision);
  outp=zeros(1,sx(2));
  for jj=1:sx(2)
   [out(jj),outp(jj)]=minOfVector(x(:,jj));
...
elseif nargin==3
 if ~isempty(y)
  error('min must have an empty 2nd arg if there are 3 input args')
 else
  if dim==1
   [out,outp]=min(x);
  elseif dim==2
...
function [out,outp]=minOfVector(in)
...
[out,outp]=minOfTwo(in(1),1,in(2),2);
for ii=3:length(in)
 [out,outp]=minOfTwo(out,outp,in(ii),ii);
end



function [out,outp]=minOfTwo(in1,in1p,in2,in2p)
