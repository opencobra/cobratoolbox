...
%
% OPTIONAL INPUT
%   dir                         Production = 1 (defauld = production), else consumption = 0
...
for k =1:length(samples)
   ...
   %[solBMa,LPProblem]=solveCobraLPCPLEX(submodel,1,0,0,[],1e-6);
    ...
    %make rxns list producing met in starting model
    for m = 1:length(met2test)
        if k == 1
            MIDX = strcmp(met2test(m),model.mets);%,'exact'); %find location of atp in S matrix
            RxnsX = find(model.S(MIDX,:)~=0); %find all reactions that contain atp
            RxnsNamesX(:,1)= model.rxns(RxnsX);
            metRall = [metRsall; RxnsNamesX];
