...
pathSaveLocation = ' ... ADD YOUR PATH...\'; % output is saved to this location, can be the same as pathToCOBRA 'C: ... \cobratoolbox\metabotools\tutorial_II\';
...
if isequal(exist([pathToCOBRA filesep 'cobratoolbox' filesep 'tutorials' filesep 'metabotools' filesep 'tutorial_II' filesep 'starting_model.mat'], 'file'),2)% 2 means it's a file.
    % We have a file!
    load([pathToCOBRA filesep 'cobratoolbox' filesep 'tutorials' filesep 'metabotools' filesep 'tutorial_II' filesep 'starting_model.mat']);
...
A = rand(1);
[pathSaveLocation filesep 'A.mat']
save([pathSaveLocation filesep 'A.mat'], 'A');
...
clearvars -EXCEPT pathSaveLocation pathToCOBRA modelMedium tol solver solverQuant

%% Section 2 - Generate an individual exchange profiles for each sample

[pathToCOBRA filesep 'cobratoolbox' filesep 'tutorials' filesep 'metabotools' filesep 'tutorial_II' filesep 'tutorial_II_data']

load([pathToCOBRA filesep 'cobratoolbox' filesep 'tutorials' filesep 'metabotools' filesep 'tutorial_II' filesep 'tutorial_II_data']);
model = modelMedium;
test_max = 500;
test_min = 0.0001;
variation = 20;

prepIntegrationQuant(model,metData,exchanges,samples,test_max,test_min,pathSaveLocation,tol,variation);

clearvars -EXCEPT modelMedium samples tol solver pathSaveLocation solverQuant pathToCOBRA

%% Section 2B - Prepare table to check exchange profiles
nmets = 70;
[mapped_exchanges, minMax,mapped_uptake, mapped_secretion] = checkExchangeProfiles(samples, pathSaveLocation ,nmets);

clearvars -EXCEPT pathSaveLocation pathToCOBRA modelMedium samples tol solver mapped_exchanges solverQuant

save ([pathSaveLocation filesep 'Result_checkExchangeProfiles']);
...
[ResultsAllCellLines,OverViewResults] = setQuantConstraints(model,samples,tol,minGrowth,obj,no_secretion,no_uptake,medium,addExtraExch,addExtraExch_value,pathSaveLocation);
clearvars -EXCEPT pathSaveLocation pathToCOBRA modelMedium samples ResultsAllCellLines OverViewResults tol solver mapped_exchanges
...
clearvars -EXCEPT modelMedium samples ResultsAllCellLines OverViewResults Ex_added_all_unique tol solver mapped_exchanges pathSaveLocation pathToCOBRA


[Added_all] = mkTableOfAddedExchanges(ResultsAllCellLines,samples,Ex_added_all_unique);

save([pathSaveLocation filesep 'statistics']);
...
[genes,ResultsAllCellLines,OverViewResults] = analyzeSingleGeneDeletion(ResultsAllCellLines, pathSaveLocation, samples, cutoff, OverViewResults);

clearvars -EXCEPT pathSaveLocation pathToCOBRA modelMedium samples ResultsAllCellLines OverViewResults Ex_added_all_unique genes tol solver mapped_exchanges
...
clearvars -EXCEPT pathSaveLocation pathToCOBRA modelMedium samples ResultsAllCellLines OverViewResults Ex_added_all_unique genes FBA_Rxns_KO ListResults tol solver mapped_exchanges
...
mk_intersect = 1;
mk_reactionDiff = 1;
load([pathToCOBRA filesep 'cobratoolbox' filesep 'tutorials' filesep 'metabotools' filesep 'tutorial_II' filesep 'starting_model']);
...
clearvars -EXCEPT pathSaveLocation pathToCOBRA modelMedium samples ResultsAllCellLines OverViewResults Ex_added_all_unique genes FBA_Rxns_KO ListResults unionModel intersectModel  diffRxns diffExRxns tol solver mapped_exchanges model

save([pathSaveLocation filesep 'summary']);
...
save([pathSaveLocation filesep 'fluxSplits']);
...
make3Dplot(PHs, maximum_contributing_flux_ATP, fonts, pathSaveLocation, diff_view);
...
save([pathSaveLocation filesep 'PPP']);
...
fonts = 12;
samples = {'IGROV1'};
illustrate_ppp(ResultsAllCellLines,mets,pathSaveLocation,samples,label,fonts,tol);
