...
% model Recon3DModel_Dec2017. However, only the data for the former is provided 
% within the COBRA Toolbox as it is used for testing von Bertylanffy, while Recon3D 
% is not yet published and the data is not yet available. Having said this, the 
% figures generated below are most suited to plotting results for Recon3D, so 
% they may not be so useful for iAF1260.  The Recon3D example uses values from 
