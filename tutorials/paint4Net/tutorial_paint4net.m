...
% representation of information in the layout allows quick assessment 
...
% 
% * [involvedMets, deadEnds] = draw_by_rxn(model, rxns, drawMap, direction, 
% initialMet, excludeMets, flux);
% * [directionRxns, involvedMets, deadEnds] = draw_by_met(model, metAbbr, drawMap, 
% radius, direction, excludeMets, flux).
%
% Application of command draw_by_rxn
% draw_by_rxn can be performed using option A in case COBRA model optimization 
% results have to be visualized or option B if the interconnections between 
% metabolites in COBRA file have to be visualized.
%
% A. COBRA model visualization 
% *i. a COBRA model has to be loaded into MATLAB. 
% 
% *ii. optimization of the objective function by using the COBRA toolbox 
% command optimizeCbModel, where the argument model corresponds to the COBRA 
% model in the MATLAB workspace.

FBAsolution = optimizeCbModel(model)
%% 
% This step ensures that a vector of the steady state fluxes x will be 
% available for the command draw_by_rxn.

FBAsolution.x
%%
% *iii. execution of the command [involvedMets, deadEnds] = draw_by_rxn 
% (model, rxns, drawMap, direction, initialMet, excludeMets, flux).
...
% *i. a COBRA model has to be loaded into MATLAB.
% 
% *ii. execution of the command [involvedMets, deadEnds] = draw_by_rxn 
% (model, rxns, drawMap, direction, initialMet, excludeMets, flux)

[involvedMets, deadEnds] = draw_by_rxn(model, model.rxns, 'true')
 
% In this case in the brackets in the rectangles are shown characters x 
...
% The command draw_by_rxn has several input arguments – model, rxns, drawMap, 
% direction, initialMet, excludeMets, and flux. The last 5 arguments are 
% optional; it means that the algorithm of the command draw_by_rxn uses default 
...
% time from scratch it is possible to create a cell type vector in the MATLAB 
% workspace that contains the static abbreviations of the reactions in the COBRA 
% model and use it as argument rxns. 
...
% accessed by model.rxns. It illustrates all COBRA model.

[involvedMets, deadEnds] = draw_by_rxn(model, model.rxns)
%% 
% iii. argument drawMap
% 
% It is a logical type variable that can take value of 'true' or 'false' 
...
% 
% It is a string type variable that can take value of 'struc', 'sub', 
% 'prod' or 'both' (default is 'struc') indicating a direction for the algorithm 
% of the command draw_by_rxn.

[Involved_mets, Dead_ends] = draw_by_rxn(model, model.rxns, 'false', 'struc')

% In case of 'struc' (structure) the algorithm visualizes all metabolites 
% connected to the specified reactions in the argument rxns. The key feature 
...
% In case of 'sub' (substrates) the algorithm visualizes only those metabolites 
% which are substrates for the specified reactions in the argument rxns. This 
...
% than +10-9 mmol*g-1*h-1 are non-zero fluxes. In case of 'prod' (products) 
% the algorithm visualizes only those metabolites which are products for the specified 
% reactions in the argument rxns but in case of 'both' the algorithm visualizes 
% both – substrates and products - for the specified reactions in the argument 
% rxns. For both cases the algorithm is using the same rules regarding to calculation 
% of the directions for each reaction as for case of 'sub'. This argument is 
% essential for the command draw_by_met of the Paint4Net v1.3 because the 
% command draw_by_met is calling out the command draw_by_rxn and passing the 
% argument direction.
%% 
% *iiiii. optional argument initialMet
% 
% It is a cell type variable that can take a value that represents the 
% abbreviation of a metabolite in the COBRA model (default is empty).

[Involved_mets, Dead_ends] = draw_by_rxn(model, model.rxns, 'true', 'struc', {'atp[c]'}, {''}, FBAsolution.x)

% This metabolite is represented as green ellipse on the map
% and this feature is essential for the command draw_by_met of the Paint4Net 
% v1.3 because the command draw_by_met is calling out the command draw_by_rxn 
% and passing the argument initialMet.
%% 
...
% the list every time from scratch it is possible to create a cell type vector 
% in the MATLAB workspace that contains the static abbreviations of the metabolites 
% in the COBRA model and use it as argument excludeMets. 
...
% 
% It is a double type Nx1 size vector of fluxes of reactions where N is 
...
% The command draw_by_rxn has two output vectors in the result: involvedMets 
% and deadEnds.
% 
% *i. the vector involvedMets
% 
% It is a cell type vector that contains a list of the involved metabolites 
% in the specified reactions.
% 
% *ii. the vector deadEnds
% 
% It is also a cell type vector but it contains a list of the dead end 
% metabolites in the specified reactions.
% 
% Application of command draw_by_met 
% draw_by_met can be used can be performed using option A in case COBRA 
% model with optimization results have to be visualized or option B if the interconnections 
% between metabolites in COBRA file have to be visualized.
% 
% A. COBRA model visualization 
% *i. a COBRA model has to be loaded into MATLAB.
% 
...
% This step ensures that a vector of the steady state fluxes x will be 
% available for the command draw_by_met.
...
% In this case in the brackets in the rectangles are shown characters xindicating 
...
% The command draw_by_met has several input arguments – model, metAbbr, drawMap, 
% radius, direction, excludeMets, and flux. The last 5 are optional, it means 
% that the algorithm of the command draw_by_met uses default values of those 
...
% 
% It is a cell type variable that can take a value that represents the 
% abbreviation of a metabolite in a COBRA model. This argument is the start 
% point for the algorithm of the command draw_by_met for visualization.
...
% 
% It is a double type variable that can take a value of natural numbers 
% (1,2,3…n).

[directionRxns, involvedMets, deadEnds] = draw_by_met(model, {'etoh[c]'}, 'true', 1)

% The argument radius indicates the depth of an analysis of the initial 
% metabolite (the argument metAbbr) and it is tightly connected to the optional 
% argument direction. For example, if user is interested in the substrates of 
% ethanol, the user can analyse substrates step by step starting from the first 
% reactions where the argument radius is equal to 1 and moving to the next reactions 
% by increasing the value of the argument radius.
% 
% The algorithm of the command draw_by_met interconnects all involved metabolites 
...
% network. The algorithm of the command draw_by_met shows the rates of fluxes 
...
% imbalance for some metabolites.
% 
% *iiiii. optional argument direction
% 
% It is a string type variable that can take value of 'struc', 'sub', 
% 'prod' or 'both' (default is 'struc') indicating a direction for the algorithm 
% of the command draw_by_met. In case of 'struc' (structure) the algorithm 
% visualizes all metabolites connected to the specified reactions in the argument 
% rxns. The key feature of this function is visualization of all specified reactions 
% not taking in account a steady state fluxes in that way representing the structure 
% of the COBRA model. In case of 'sub' (substrates) the algorithm visualizes 
% only those metabolites which are substrates for the specified reactions in the 
% argument rxns. This time the algorithm is using a stoichiometric matrix and 
...
% of 'prod' (products) the algorithm visualizes only those metabolites which 
% are products for the specified reactions in the argument rxns but in case 
% of 'both' the algorithm visualizes both – substrates and products - for the 
% specified reactions in the argument rxns. For both cases the algorithm is 
...
% number of reactions (default is vector of x). This vector is calculated during 
% the optimization of the objective function and can be accessed through the result 
% of the optimization command by FBAsolution.x. 
%
% Output of the command draw_by_met 
% The command draw_by_met has three output vectors in the result: involvedRxns, 
% involvedMets, and deadEnds.
% 
% *i. vector involvedRxns
% 
% It is a cell type vector that contains a list of the involved reactions 
% according to the set of input arguments.
% 
...
% 
% *Solution: *It is a logical type variable that can take value of 'true' 
% or 'false' (default is 'false') indicating whether to visualize the COBRA 
...
% and the list of them will be created in the MATLAB workspace as variable involvedMets. 
...
% workspace as variable involvedMets. The expected dead-end metabolites are: 
...
% Toolbox. The reaction nodes will contain x for flux rates.
...
% the hypergraph layout will be generated by Paint4Net using the Bioinformatics 
% Toolbox. The reaction nodes will contain x for flux rates. All interconnecting 
...
% 
% The list with atp, nad, adp and h as excludable metabolites will 
% be created in the MATLAB workspace as variable excludeMets.
...
% reaction nodes will contain ”x” for flux rates. The generated layout will not 
% contain any of metabolites declared in the list of excludable metabolites excludeMets. 
...
% of excludable metabolites excludeMets.
...
% directionRxns. The expected involved metabolites are: 'f6p[c]', 'g6p[c]', 
...
% involvedMets. The expected dead-end metabolites are: 'f6p[c]', 'glc-D[c]', 
...
% workspace as variable deadEnds.
...
% will contain x for flux rates. All interconnecting edges will be in gray because 
% of no FBA data.
