...
surfNet([], 'GLCtex_copy1', 0, 'none', 0, 1, [], 0)  % called by clicking 'GLCtex_copy1'
%% 
...
surfNet([], 'glc__D_p', 0, 'none', 0, 1, [], 0)  % called by clicking 'glc__D_p'
%% 
% Click GLCptspp:

surfNet([], 'GLCptspp', 0, 'none', 0, 1, [], 0)  % called by clicking 'GLCptspp'
%% 
% Click g6p_c:

surfNet([], 'g6p_c', 0, 'none', 0, 1, [], 0)  % called by clicking 'g6p_c'
%% 
...
surfNet([], [], 0, 'none', 0, 1, [], 0, struct('showPrev', true))  % called by clicking 'Show previous steps...'
%% 
...
iJO1366 = changeRxnBounds(iJO1366, {'EX_glc__D_e'; 'EX_fru_e'}, [0; -10], {'L'; 'L'});
printUptakeBound(iJO1366);
%% 
...
rxnDiff = abs(fluxMatrix(:, 1) - fluxMatrix(:, 2)) > 1e-6;  % reactions with different fluxes
...
% to #mets or #rxns) or from the name of the field (starting with 'met' or 'rxn'), 
% the fields are recognizable to be met- or rxn-related. For example, show the 
% grRules for rxns but omit the bounds and show the constraint sense (csense) 
% associated with each metabolite. Note the difference from the original call:

surfNet(iJO1366, 'fdp_c', [], [], [], [], {'metNames','metFormulas','rxnNames','grRules','csense'})
