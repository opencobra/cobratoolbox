 ...
 *    Author: Steinn Gudmundsson, 10.7.2013
 *
 *    A MEX C++ function for performing flux variability analysis with the CPLEX solver.
 *
 ...
 *
 *    Optional output (takes up a lot of memory):
 *       fbasol    Flux vectors for the initial FBA
 *       fvamin    Matrix of flux values for each individual minimization
 *       fvamax    MAtrix of flux values for each individual maximization
 *
 * Example:
 ...
 *   mex -largeArrayDims -I"C:\Program Files\IBM\ILOG\CPLEX_Studio125\cplex\include\ilcplex" cplexFVAc.c "C:\Program Files\IBM\ILOG\CPLEX_Studio125\cplex\lib\x64_windows_vs2008\stat_mda\cplex125.lib" "C:\Program Files\IBM\ILOG\CPLEX_Studio125\cplex\lib\x64_windows_vs2008\stat_mda\ilocplex.lib"
 *
...
enum {MINFLUX_OUT_POS, MAXFLUX_OUT_POS, OPTSOL_OUT_POS, RET_OUT_POS, 
      FBA_SOL_OUT_POS, FVA_MIN_OUT_POS, FVA_MAX_OUT_POS, MAX_NUM_OUT_ARG};
...
int _fva(CPXENVptr env, CPXLPptr lp, double* minFlux, double* maxFlux, double* optSol,
         double* FBAsol, double* FVAmin, double* FVAmax,
         mwSize n_constr, mwSize n_vars, 
   ...
   for (iRound = 0; iRound < 2; iRound++)
   {
      ...
      for (k = 0; k < nrxn; k++)
      {
         ...
         if (FVAmin != NULL && FVAmax != NULL)
         {
            ...
            //for (int iflux = 0; iflux < n; iflux++)
            //{
            //   ptr[iflux + k*n] = glp_get_col_prim(P, iflux + 1);
            //}
         }
    ...
    */
    ...
    if (nrhs < MIN_NUM_IN_ARG) {
        ...
        sprintf(errmsg, "At least %d output arguments required.",
                MIN_NUM_OUT_ARG);
        TROUBLE_mexErrMsgTxt(errmsg);
    } else if (nlhs > MIN_NUM_OUT_ARG && nlhs != MAX_NUM_OUT_ARG)
    {
       TROUBLE_mexErrMsgTxt("Exactly seven output arguments are required to get flux values.");
    } else if (nlhs > MAX_NUM_OUT_ARG) {
        TROUBLE_mexErrMsgTxt("Incorrect number of output arguments.");
    }
    ...
    // Use Primal Simplex always (needed for CPLEX version 12.4 and later)
    status = CPXsetintparam (env, CPX_PARAM_LPMETHOD, CPX_ALG_PRIMAL);
    if ( status ) {
        mexPrintf("Failure to set LPMETHOD parameter.\n");
        dispCPLEXerror(env, status);
        goto TERMINATE;
    }
    ...
    /*
       Create matrices for the four main return arguments.
    */
